...
\begin{itemize}
    ...
    基本型と合成型がある。基本型は型の最小構成要素となる定義であり、合成型は基本型の組み合わせによってできる型である。
    ...
    インスタンス変数定義では、各オブジェクトがオブジェクト内に保持する属性を定義する。
...
本研究では、
Markdownで記述した画面遷移システムの仕様を解析することで、本研究で提案するCTMの表示、および、VDM++仕様の生成を行う。以降、本論文では、Markdownで記述した画面遷移システムの仕様を、Markdown仕様と呼ぶ。
...
CTM要素の配置情報(\ref{sec:ProjectManagementComponent}節を参照)の保持にJSONを用いる。
...
各メソッドにRelayCommand属性を付与することにより、拡張後の\tool 内でそのメソッドが非同期実行のコマンドとして扱えるため、重い処理を行っていてもUIを止めずにバックグラウンドで処理を実行できる。
...
本研究の拡張で使用する.NET MAUIの名前空間を、以下に示す。
...
\begin{itemize}
    ...
    本研究では、 XAML ファイルに画面レイアウトを宣言的に記述し、画面構造と処理ロジックの分離を実現する。
...
当該手法に基づく解析、および、\VDM 仕様への変換処理を行うツールである。
本節では、\tool が採用している
...
\begin{itemize}
\item システムに存在する画面の名前をすべて記載した、画面一覧仕様が存在する。
\item 各画面に1対1で対応した、画面仕様が存在する。
\end{itemize}
...
表\ref{tab:gamenichiran}における【】で囲んだ部分は、対象とする画面一覧仕様の記述に依存する部分であることを表す。
...
この変換ルールBを、表\ref{tab:kakugamen}に示す。表\ref{tab:kakugamen}における【】で囲んだ部分は、対象とする画面仕様の記述に依存する部分であることを表している。
