...
\item 同一画面内の別状態への変化
\item 別の画面への遷移
\item 条件付きで変化先が決定される内部状態または画面への遷移
\end{itemize}
\end{itemize}
...
本研究では、画面遷移システムの仕様をVDM\texttt{++}として出力する。また、VDM\texttt{++}を、
...
\renewcommand{\arraystretch}{1.3} % セルの上下に少し余裕を持たせる
\begin{tabular}{|l|l|}
\hline
...
引用 & \texttt{> 引用} \\ \hline
リンク & \texttt{[リンク名](リンクURL)} \\ \hline
コード & \texttt{\`}\texttt{コード}\texttt{\`} \\ \hline
コードブロック & \begin{tabular}{@{}l@{}} \texttt{\`}\texttt{\`}\texttt{\`} \\ \texttt{コードブロック} \\\texttt{\`}\texttt{\`}\texttt{\`} \end{tabular} \\
\hline
\end{tabular}
\end{table}
...
Markdownで記述した画面遷移システムの仕様を解析することでGUI表示、および、VDM++仕様の生成を行う。
...
GUI配置の内部表現としてJSONを扱う。その後JSONの読み出し、および、書き込みを行い、ツールのGUIやVDM\texttt{++}仕様生成に用いる。
...
Popup は、画面全体を遷移させることなく、一時的な入力画面や確認画面を表示するための UI コンポーネントである。
...
\begin{itemize}
    ...
    .NET MAUI においてユーザインタフェースを操作するためのメインスレッドを扱うための API であるMainThreadを用いて、UIスレッド上での処理制御を行っている。
...
\begin{itemize}
    ...
    正規表現による文字列解析機能を提供する.NET 標準ライブラリ。本研究では、Markdown 形式で記述した画面遷移システム仕の様を解析する際に用いている。
    \item System.Collections.Generic：

    ジェネリックコレクションを提供する.NET 標準ライブラリ。本研究では、リストを管理するために用いている。
    \item System.IO：
...
本手法が対象とする画面遷移システムの仕様の構成を、以下に示す。
...
本手法でVDM\texttt{++}へ変換を行う画面一覧仕様の例を図\ref{fig:specRuleA}、画面仕様の例を図\ref{fig:specRuleB}にそれぞれ示す。
また、定義する表示部クラスの例を、リスト\ref{lst:hyougibu}に、VDM\texttt{++}に変換後の画面管理クラスの例を、リスト\ref{lst:VDMPP_ExampleA}に、画面クラスの例を、
リスト\ref{lst:VDMPP_ExampleB}にそれぞれ示す。
\begin{figure}
    \centering
...
\begin{figure}[tp]
    \begin{lstlisting}[caption={図\ref{fig:specRuleA}に対応する画面クラス}, label={lst:VDMPP_ExampleA}, language={VDM_PP}]
class (*@「@*)画面管理(*@」@*)
...
\begin{itemize}
    ...
    自然言語仕様記述ルールに基づいた画面遷移システム自然言語仕様から、VDM++仕様記述への変換ルール。
    以下の2つのルールがある。
    \begin{itemize}
        \item 変換ルールA\\
...
\begin{itemize}
    \item Markdown形式で記述した画面遷移システム仕様(以降、Markdown仕様と呼ぶ)の編集機能
