...
\item 同一画面内の別状態への変化
\item 別の画面状態への遷移
\item 条件付きで変化先が決定される内部状態または画面状態への遷移
\end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    関数定義では、オブジェクトの振る舞いの一部を定義する。関数は引数の値だけで戻り値が定まり、インスタンス変数を参照することができない。
    VDM\texttt{++}の関数は、関数が何をすべきかの特性のみを記述する陰関数定義と、引数からどのように結果を計算すべきかアルゴリズムを示す陽関数定義の2つの定義スタイルがある。

    \item 操作定義

    操作定義では、オブジェクトの振る舞いの一部を定義する。操作は入力引数を受け取り結果を返し、そのオブジェクト内の、あるいは参照しているオブジェクトのインスタンス変数を参照することができる。
...
引用 & \texttt{> 引用} \\
\hline
リンク & \texttt{[リンク名](リンクURL)} \\
\hline
コード & \texttt{`}コード\texttt{`} \\
\hline
コードブロック &
\parbox[t][12.0ex][t]{\linewidth}{%
...
CommunityToolkit.Mvvmは、高速モジュール式 MVVM ライブラリである。本ライブラリは、アプリを構築するための最初の実装を提供する、
標準型、自己完結型、軽量型のコレクションにアクセスするために使用する\cite{MVVMtool}。

本研究では、CommunityToolkit.MvvmのRelayCommandを主に使用している。
...
本研究では、CommunityToolkit.Maui が提供する \verb|Popup| 機構を主に利用している。
...
\begin{itemize}
    ...
    .NET MAUI においてユーザインタフェースを操作するためのメインスレッドを扱うための API であるMainThreadを用いたUIスレッド上での処理制御に利用している。
...
\begin{itemize}
    ...
    正規表現による文字列解析機能を提供する.NET 標準ライブラリ。本研究では、Markdown 形式で記述された画面遷移システム仕様を解析する際に用いている。
    \item System.Collections.Generic：

    ジェネリックコレクションを提供する.NET 標準ライブラリ。本研究では、内部表現を管理するために用いている。
    \item System.IO：
...
本手法でで作成を行う画面遷移システム仕様の構成を、以下に示す。
...
本手法では、VDM\texttt{++}による画面遷移システム仕様の構成は、システムにおける表示部分を表現する表示部クラス、
現在画面の状態を管理する画面管理クラス、および、画面一つ一つの仕様に基づく画面定義を表現する画面クラスから成る。
なお、本手法において表示部クラスは、対象の仕様書の内容にかかわらず一定のものとして定義する。

本手法でVDM\texttt{++}へ変換を行う画面一覧仕様の例を図\ref{fig:specRuleA}、画面仕様の例を図\ref{fig:specRuleB}に示す。
また、定義する表示部クラスの例を、リスト\ref{lst:hyougibu}に、VDM\texttt{++}に変換後の画面管理クラスの例を、リスト\ref{lst:VDMPP_ExampleA}に、画面クラスの例を、リスト\ref{lst:VDMPP_ExampleB}に示す。
\begin{figure}
    \centering
...
\begin{itemize}
    \item 自然言語仕様記述ルール\\(\ref{sec:Specrule}で後述)
...
\begin{enumerate}
  \item 自然言語仕様記述ルールに基づいて、画面遷移システム仕様を作成する。
...
画面一覧仕様の自然言語仕様記述ルールを、以下に示す。図\ref{fig:specRuleA}に示した画面管理仕様は、以下のルールに則て記述している。
\begin{itemize}
...
画面仕様の自然言語仕様記述ルールを、以下に示す。図\ref{fig:specRuleB}に示した画面仕様は、以下のルールに則て記述している。
...
先が異なる場合は、イベント時分岐記述フィールドに記述を追加する必要がある。

\subsection{VDM\texttt{++}変換ルール}\label{sec:ConvRule}
本節では、自然言語仕様記述ルールに基づいた画面遷移システム仕様からVDM++への変換ルールA、および、
変換ルールBを用いてた、arkdown仕様からVDM\texttt{++}への変換について説明する。

\subsubsection{画面管理クラス}

画面管理クラスは、現在の画面の状態を管理するクラスであり、画面管理を表現する。
画面遷移システム仕様からVDM++へ変換を行う際、画面一覧仕様から画面管理クラスへの変換ルール(変換ルールA)に則って、記述を行う。
...
\subsubsection{画面クラス}
画面クラスは、画面の仕様に基づく画面定義を表現する。
画面遷移システム仕様からVDM++へ変換を行う際、画面仕様から画面クラスへの変換ルール(変換ルールB)に則って、記述を行う。
