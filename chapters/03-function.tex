...
CTMの例を図\ref{fig:ctm_example}に示す。
具体的には、CTMは、画面遷移システムにおけるボタンとイベントの関係を表す。
本研究では、ユーザによるCTMの編集を基に、
Markdown仕様、および、VDM\texttt{++}仕様を生成することで、
VDM\texttt{++}仕様の視覚的編集を可能とする。
\begin{figure}[tp]
  \centering
  \includegraphics[width=0.9\linewidth]{./images/CTM_Example.png}
  \caption{図\ref{fig:specRuleB}に対応するCTMの例}
...
\begin{itemize}
  ...
  \item イベント要素
  \begin{itemize}
    \item イベント要素は、ある画面遷移システムにおいて、対象の有効ボタンの押下時に発生する動作を表し、矩形で表現する。
    \item 対象の有効ボタンを表すボタン要素に矢印で接続した状態で描画する。
    \item イベント動作名を保持する。
  \end{itemize}
  \item 分岐イベント要素
  \begin{itemize}
    \item 分岐イベント要素は、ある画面遷移システムにおいて、対象の有効ボタン押下時に条件付きで発生する動作を表し、その条件を、分岐条件として、ダイヤモンド形状で描画する。
    また、条件付きで発生する動作を、分岐先イベントとして、矩形で表現する。
    \item 対象の有効ボタンを表すボタン要素に矢印で接続した状態で描画する。
    \item 分岐条件名、および、分岐先イベント動作名を保持する。
  \end{itemize}
  \item タイムアウト要素
  \begin{itemize}
    \item タイムアウト要素は、ある画面遷移システムにおいて、一定時間内に特定の操作が行われなかった場合に発生する動作を表し、
    CTM上ではタイムアウト時間をピンク色の楕円、タイムアウト後遷移先をピンク色の矩形で表現する。
    \item タイムアウト時間、および、タイムアウト後遷移先名を保持する。
    \end{itemize}
  \item 遷移先のないイベント要素
  \begin{itemize}
    \item 画面遷移を伴うイベントで遷移先のないイベント要素は、イベント動作に対応する画面要素が画面管理クラスに存在しない場合に表示する要素であり、赤の矩形で強調して表現する。
    \item イベント動作名を保持する。
    \end{itemize}
\end{itemize}
...
\begin{table}[tp]
  ...
  \begin{tabular}{|>{\centering\arraybackslash}m{5cm}|>{\centering\arraybackslash}m{6cm}|}
    ...
    \hline
    分岐イベント & \includegraphics[width=0.9\linewidth]{./images/条件分岐イベント.png} \\
    \hline
...
「スタートページ」は「Markdown仕様記述ページ」(\ref{sec:MarkdownPage}節を参照)と「GUI操作によるVDM\texttt{++}仕様編集ページ」(\ref{sec:NoCodePage}節を参照)へ遷移する以下の2つの切り替えボタンを表示し、
...
「Markdown仕様記述ページ」は既存の\tool が保持している画面である。既存部分の詳細は\ref{sec:2vdm-spec-generator}節に示す。
...
\section{GUI操作によるVDM\texttt{++}仕様編集ページ}\label{sec:NoCodePage}

「GUI操作によるVDM\texttt{++}仕様編集ページ」は、本研究の拡張で新たに追加するページである。
図\ref{fig:initial_display}に示した「GUI操作によるVDM\texttt{++}仕様編集ページ」の外観に、領域ごとに枠を付与したものを、図\ref{fig:NoCodePageScreen}に示す。
...
\end{figure}
各領域と機能の対応表を表\ref{tab:area_func}に示す。
...
\begin{table}[tp]
  ...
  \begin{tabular}{|c|c|}
    ...
    \textbf{領域} & \textbf{機能} \\
    \hline
    フォルダツリー表示領域 &  フォルダツリー描画機能 \\
    \hline
    VDM\texttt{++}仕様表示領域 & VDM\texttt{++}仕様描画機能 \\
    \hline
    メニューバー領域 & メニューバーの操作による仕様生成補助機能 \\
    \hline
    CTM領域 & 
    \begin{tabular}{c}
    CTM描画機能 \\
    CTM領域上でのユーザ操作によるCTM編集機能
    \end{tabular}\\
    \hline
    操作ボタン領域 & 
    \begin{tabular}{c}
    操作ボタン描画機能 \\
    操作ボタン領域のボタン操作によるCTM編集機能
    \end{tabular}\\
    \hline
  \end{tabular}
\end{table}
...
以降、「GUI操作によるVDM\texttt{++}仕様編集ページ」の各機能について、以下で詳細に示す。
\subsection{描画機能}
描画機能は、以下の4つの機能から成る。
\begin{itemize}
  \item CTM描画機能
...
CTM描画機能は、自然言語仕様記述ルール(\ref{sec:Specrule}節を参照)に沿ったMarkdown仕様に
対応するCTMを、自動でCTM領域に描画する機能である。本機能は、ユーザがMarkdownファイルの選択を行った際、および、ユーザがCTMの編集を行った際に、CTM領域にCTMを描画する。

具体例は図\ref{fig:ctm_example}に示している。

本機能では、Markdown仕様ファイルを解析し、表\ref{tab:ctm_elements}に示したCTMの各要素を抽出してCTM領域に描画する。
...
ユーザは、フォルダツリー表示領域に表示するアイテムの対象は、フォルダ、および、拡張子が.mdのファイルである。
ツリーに表示するフォルダには、フォルダ名の左側にフォルダアイコンを表示する。
フォルダツリー表示領域の表示例を、図\ref{fig:folder}に示す。

ユーザは、フォルダツリー表示領域に表示するアイテムを選択することで、フォルダの展開、または、編集対象のMarkdownファイルを選択することが可能である。
...
VDM\texttt{++}仕様描画機能は、VDM\texttt{++}仕様表示領域に、フォルダツリー表示領域において、ユーザが編集対象に選択したMarkdown仕様ファイルに対応するVDM++仕様を描画する機能である。
本機能は、CTM描画機能と同じタイミングで実行する。
つまり、\tool を操作することによるVDM++仕様への変更は即時反映し、常に最新状態のVDM\texttt{++}仕様をVDM\texttt{++}仕様表示領域に描画する。
VDM\texttt{++}仕様表示領域の表示例を、図\ref{fig:VDMEX}に示す。
...
この機能により、ユーザによるプロジェクトフォルダ選択の有無、および、選択したMarkdown仕様ファイルに応じて、実行可能な操作ボタンのみを操作ボタン領域に描画し、ユーザによる不適切な操作の実行を防止する。
本機能は、ユーザがフォルダツリー表示領域内のMarkdown仕様ファイルを選択した際に、実行する。
操作ボタンの表示パターンは、「GUI操作によるVDM\texttt{++}仕様編集ページ」への遷移直後、および、編集対象となっているVDM\texttt{++}仕様のクラスの種類に基づいて、
以下の4種類に分類する。
...
各表示パターンとそれぞれの表示パターンを表示する条件の対応を表\ref{tab:Look_patterns}に、操作ボタンと各操作ボタン押下時処理の対応を表\ref{tab:Look_patterns_operations}にそれぞれ示す。

各表示パターンについて、以下で説明する。

\paragraph{表示パターンA}

表示パターンAは、「スタートページ」から「GUI操作によるVDM\texttt{++}仕様編集ページ」への遷移直後で、編集対象となるプロジェクトフォルダを選択していない場合の表示パターンである。
表示パターンAは、ユーザによる、編集対象となるプロジェクトフォルダの設定を目的としている。
...
表示パターンDは、自然言語仕様記述ルールに従っていないMarkdown仕様を編集対象とした場合の表示パターンである。
表示パターンDは、ユーザによるMarkdown仕様の種類が定まっていないままでの操作実行を防止することを目的としている。
表示パターンDに表示する操作ボタンは、以下の2つ。
...
\begin{table}[tp]
  \centering
  \caption{操作ボタンとボタンに対応する機能および各操作ボタン押下時処理の対応}
  \label{tab:Look_patterns_operations}
  \begin{tabular}{|c|c|p{5.5cm}|}
    \hline
    \textbf{ボタン} &\textbf{機能}& \textbf{各操作ボタン押下時処理} \\
    \hline
    フォルダの選択 &フォルダの選択&
    ...
    \hline
    クラス名(画面名)の変更 &クラス名(画面名)の変更&
    選択中の画面クラス名を変更する。 \\
    \hline
    クラスの種類選択・追加 &クラスの種類選択および追加&
    画面管理クラスか画面クラスかを選択、仕様のクラス名を定義する。 \\
    \hline
    削除 &削除&
    選択中の要素を削除する。 \\
    \hline
    スタートページへ戻る &スタートページに戻る&
    スタートページへ遷移する。 \\
    \hline
  \end{tabular}
\end{table}
...
その結果を\tool の記述ルールに沿ったMarkdown仕様、および、
VDM\texttt{++}仕様に変換する機能である。

以降、各機能の詳細を示す。

\subsubsection{メニューバーの操作による仕様生成補助機能}
メニューバーの操作による仕様生成補助機能は、メニューバーに表示する、VDM\texttt{++}仕様生成に必要な、ファイル、および、フォルダに関する機能を操作することで、VDM\texttt{++}仕様生成の補助を行う機能である。
...
フォルダ選択インターフェースで操作対象のプロジェクトフォルダを選択し、フォルダツリー表示領域に選択したプロジェクトフォルダのアイテムを表示する。
この操作で選択したプロジェクトフォルダに関して、拡張した\tool では編集を行う。

\paragraph{新規ファイル作成}

「新規ファイル作成機能」は、空のMarkdownファイルを作成し、プロジェクトフォルダに追加する機能である。
なお、この際にVDM++ファイルは作成しない。
本機能では、ユーザが「新規ファイルの作成」項目を押した際に、新規ファイル作成ダイアログを表示する。
...
\end{figure}
新規ファイル作成ダイアログにファイル名を入力することで、新規Markdownファイルを作成する。
...
表示するクラスの種類選択ダイアログから、追加するクラスの種類を選択する。クラスの種類選択ダイアログを、図\ref{fig:class_type_dialog}に示す。
ユーザが「画面管理クラスの追加」を選択した場合は、編集対象のMarkdown仕様の1行目に「\# 画面一覧」を追加、および、
対応するVDM++仕様のクラス名を「画面管理」とし、表示パターンBへ切り替える。
ユーザが「画面クラスの追加」を選択した場合は、クラス名を入力する、クラス追加ダイアログを表示する。画面クラス追加ダイアログを、図\ref{fig:screen_name_dialog}に示す。
ユーザがクラス名を入力後、「OK」をクリックすることで、編集対象のMarkdown仕様の1行目に「\# \# 入力したクラス名」を追加、および、
対応するVDM++仕様のクラス名を入力したクラス名とし、表示パターンCへ切り替える。

画面一覧仕様であるMarkdown仕様、および、画面管理クラスであるVDM\texttt{++}仕様がすでにプロジェクトフォルダ内に存在する場合は、クラスの種類選択ダイアログの表示は行わず、
直接画面クラス追加ダイアログを表示する。ユーザがクラス名の入力を完了すると、画面一覧仕様であるMarkdown仕様、および、画面管理クラスであるVDM\texttt{++}仕様に同様の画面を追加する。
\begin{figure}[tp]
  \centering
  \begin{minipage}[c]{0.45\linewidth}
    \centering
    \includegraphics[width=0.8\linewidth]{./images/1-01.png}
    \caption{クラスの種類選択ダイアログ}
...
入力した画面名に基づいて、
CTM上に新たな画面要素を生成し、
画面管理クラスのVDM\texttt{++}仕様にも対応する画面を追加する。画面追加ダイアログを、図\ref{fig:screen_name_dialog_add}に示す。
画面追加後の具体例を図\ref{fig:add_screen}に示す。
...
本機能により、画面遷移システムの特定の画面におけるユーザ操作を定義できるようにする。
なお、本機能は、表示パターンCにおいてのみ有効である。

ユーザは「ボタンの追加」ボタンをクリックした際に、
ボタン追加ダイアログを表示する。
そしてユーザが追加するボタン名を入力し、「OK」をクリックすることで、
入力したボタン名に基づいて、
CTM上に新たな画面要素を生成し、
画面クラスのVDM\texttt{++}仕様にも対応するボタンを追加する、図\ref{fig:button_name_dialog}に示す。
ボタン追加後の具体例を図\ref{fig:add_button}に示す。
\begin{figure}[tp]
  \centering
...
本機能により、画面遷移システムの特定の画面におけるタイムアウト動作を定義できるようにする。
なお、本機能は、表示パターンCにおいてのみ有効である。

本機能では、ユーザが「タイムアウトの追加」ボタンをクリックした際に
表示するタイムアウト設定ダイアログから、タイムアウト時間およびタイムアウト後遷移先を入力する。タイムアウト設定ダイアログを、図\ref{fig:timeout_setting_dialog}に示す。

ユーザが、タイムアウト時間およびタイムアウト後の動作を入力後、「OK」をクリックすることで、
入力した情報に基づいて、
CTM上に新たなタイムアウト要素を生成し、
VDM\texttt{++}仕様に対応するタイムアウトを追加する。
...
タイムアウト設定後の具体例を図\ref{fig:add_timeout}に示す。
...
新たなイベント要素を追加する機能である。
本機能により、画面遷移システムのユーザ操作に対するイベント動作を定義できるようにする。
なお、本機能は、表示パターンCにおいてのみ有効である。

ユーザが「イベントの追加」ボタンをクリックした際に、
まず、対象ボタン選択ダイアログを表示する。対象ボタン選択ダイアログを、図\ref{fig:target_button_selection_dialog}に示す。
ユーザがイベント要素を追加する対象ボタン要素を選択した後、分岐イベント選択ダイアログを表示する。分岐イベント選択ダイアログを、図\ref{fig:branch_event_selection_dialog}に示す。
ユーザが分岐イベント選択ダイアログで「はい」を選択した場合は、分岐イベント入力ダイアログを表示する。分岐イベント入力ダイアログを、図\ref{fig:branch_event_input_dialog}に示す。
ユーザが分岐イベント選択ダイアログで「いいえ」を選択した場合は、単一イベント入力ダイアログを表示する。単一イベント入力ダイアログを、図\ref{fig:normal_event_input_dialog}に示す。
分岐イベント入力ダイアログでは、イベント種別の選択、分岐条件、および、遷移先、または、操作内容を入力する。
単一イベント入力ダイアログでは、イベント種別の選択、および、遷移先、または、操作内容を入力する。

イベントの種類は、以下の4つである。
\begin{itemize}
  \item 遷移：対象ボタン押下後に特定の画面へ遷移する動作
  \item 追加：対象ボタン押下後に表示部(\ref{sec:2vdm-spec-generator}節を参照)へ、指定した文字列を追加する動作
...
ユーザが分岐イベント入力した場合は、「OK」をクリック後、別の分岐イベント追加確認ダイアログを表示する。別の分岐イベント追加確認ダイアログを、図\ref{fig:another_branch_event_dialog}に示す。
...
単一イベント追加後の具体例を図\ref{fig:add_event}に、分岐イベント追加後の具体例を図\ref{fig:add_branch}に示す。

\begin{figure}[tp]
  \centering
  \includegraphics[width=0.4\linewidth]{./images/5-05.png}
  \caption{別の分岐イベント追加確認ダイアログ}
...
編集対象にしている画面仕様、および、画面クラスのクラス名(画面名)を変更する機能である。本機能は、表示パターンCにおいてのみ有効である
...
入力した情報に基づいて、クラス名(画面名)の変更を行う。VDM\texttt{++}仕様上のクラス名も変更後のクラス名に変更する。

変更前、画面管理クラスに編集対象の画面名を定義している場合には、画面管理クラス内も変更後のクラス名(画面名)へと自動で変更を行う。
...
本機能により、ユーザは、不要なCTMの要素をCTM領域から削除し、また、その要素に対応するMarkdown仕様、および、 VDM++ 仕様内の記述を削除できる。
本機能は、表示パターンＢ、および、表示パターンＣにおいて有効である。

ユーザは、CTM領域上の要素を左クリックし削除対象とする要素を設定する。削除対象とする要素を設定後、「削除」ボタンをクリックすることでCTM領域上、Markdow仕様、および、VDM\texttt{++}仕様上から削除を行う。
...
\paragraph{フォルダの選択}
フォルダの選択は、メニューバーの操作による仕様生成補助機能で表記した「フォルダを選択」と同様の機能である。本機能は、表示パターンDにおいてのみ有効である。

ユーザは「フォルダを選択」ボタンを押した際に。図\ref{fig:fo}に示すインターフェースをより、編集対象とするプロジェクトフォルダを選択する。
ユーザがプロジェクトフォルダを選択した後、フォルダツリー表示領域に選択したプロジェクトフォルダのアイテムを表示する。
...
右クリックを行った場合、対象の要素の種類、および、条件分岐の有無に基づき、表示する操作候補(コンテキストメニュー)を決定する。各CTM要素に対するコンテキストメニューの表示例を、
図\ref{fig:context}、図\ref{fig:1context}、図\ref{fig:2context}、図\ref{fig:3context}、図\ref{fig:4context}にそれぞれ示す。
分岐イベント要素の場合は、分岐条件、および、分岐先イベントに対する右クリックを区別し、選択した岐に対する編集や削除を可能とする。
なお、右クリック操作は操作ボタンと同等の編集機能を提供し、利用者の操作導線を増やす役割を持つ。
なお、右クリックのコンテキストメニューからイベント要素の追加をする際は、すでに対象ボタンを選択しているため、
...
要素の編集は、各CTM要素の追加を行う際に表示するダイアログと同様のダイアログに編集後の内容を入力することで、
対象とするCTM要素の内容を編集する機能である。
...
コピー\&ペーストは、ボタン要素、および、画面要素に対してのみ行うことができる機能である。
...
貼り付け後の具体例を図\ref{fig:add_copy}に示す。
...
選択したCTM要素は、枠線の色や太さを変更して強調表示し、
ユーザに現在の編集対象を視覚的に示す。強調表示の例を、図\ref{fig:pick}に示す。
CTM要素が存在しない領域をクリックした場合は、選択中のCTM要素の選択状態を解除する。
...
ドラッグ操作は、CTM要素を左クリックしている間移動可能な要素(\ref{sec:GUIElementGenerationComponent}節を参照)に限り、移動できる機能である。
このドラッグ操作の結果はVDM\texttt{++}仕様上での定義の順序に関係しており、ドラッグ操作による要素の新しい配置情報はJSONファイルとして出力する。

\subparagraph{画面管理クラスから対象画面への遷移}
画面管理クラスから対象画面への遷移は、画面管理クラスのCTM要素である画面要素を左ダブルクリック、すると対象の画面の編集画面へと遷移する機能である。
なお、本機能は、画面管理クラスで定義する名称と、画面クラスで定義しているクラス名 (画面名) が完全に一致している場合に限り、使用可能である。
画面管理クラスには定義しているが、画面クラスとして存在していない場合、図\ref{fig:error}に示すエラーを表示し、機能を中断する。
\begin{figure}
  \centering
  \includegraphics[width=0.4\linewidth]{./images/error.png}
  \caption{画面管理クラスから対象画面遷移時対象画面が無い時のエラー表示}
  \label{fig:error}
\end{figure}
