...
Markdown仕様、を生成した後、\VDM 仕様を生成することで、
\VDM 仕様の視覚的編集を可能とする。
\begin{figure}[tp]
  \centering
...
以降、「GUI操作による\VDM 仕様編集ページ」の各機能について、以下で詳細に示す。
\subsection{描画機能}
描画機能は、以下の4つの機能から成る。
\begin{itemize}
  \item CTM描画機能
...
\VDM 仕様描画機能は、\VDM 仕様表示領域に、フォルダツリー表示領域において、ユーザが編集対象に選択したMarkdown仕様ファイルに対応するVDM++仕様を描画する機能である。
...
\begin{table}[tp]
  ...
  \begin{tabular}{|c|c|}
    ...
    \hline
    表示パターンD &
    クラス定義が存在しないMarkdownファイルまたはフォルダを編集対象として選択している状態 \\
    \hline
  \end{tabular}
\end{table}
...
メニューバーの操作による仕様生成補助機能は、メニューバーに表示する、\VDM 仕様生成に必要な、
ファイル、および、フォルダに関する機能を操作することで、\VDM 仕様生成の補助を行う機能であり、メニューバーに表示する。
...
また、本機能によりCTMを編集した直後、Markdown仕様、および、\VDM 仕様を最新のCTMに対応したものに更新する。
...
直接画面クラス追加ダイアログを表示する。ユーザがクラス名の入力を完了すると、画面一覧仕様であるMarkdown仕様、および、画面管理クラスである\VDM 仕様に同様の画面を追加する。
\begin{figure}[tp]
  \centering
...
入力した画面名に基づいて、
CTM上に新たな画面要素を生成し、
画面管理クラスの\VDM 仕様にも対応する画面を自動で追加する。画面追加ダイアログの外観を図\ref{fig:screen_name_dialog_add}に、
...
入力したボタン名に基づいて、
CTM上に新たなボタン要素を生成し、対応するボタンを自動で\VDM 仕様にも追加する。
...
タイムアウト後の動作を未定義として\VDM 仕様に追加する。タイムアウト設定時の入力エラーを、図\ref{fig:timeoutError}に示す。

また、画面クラスの\VDM 仕様にはタイムアウトの定義は単一のものであるため、
...
ユーザは、CTM領域上の要素を左クリックし削除対象とする要素を決定する。削除対象とする要素を決定後、ユーザが「削除」ボタンをクリックすることでCTM領域上、Markdown仕様上、および、\VDM 仕様上から対応する要素、および、定義部分の削除を行う。
...
このドラッグ操作の結果は\VDM 仕様上での定義の順序に関係しており、ドラッグ操作による要素の新しい配置情報は、ドラッグ操作ごとにJSONファイルとして出力する。
