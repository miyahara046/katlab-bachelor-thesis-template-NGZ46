...
なお、拡張後の\tool は、Windows OSにのみ対応しており、それ以外のOSには対応していない。
...
図\ref{fig:specRuleB}に示した画面仕様に対応するCTMの例を、図\ref{fig:ctm_example}に示す。
...
\begin{itemize}
  ...
  \item ボタン要素
  \begin{itemize}
    \item ボタン要素は、ある画面遷移システムにおいて、有効ボタンを表す要素であり、水色の楕円で表現する。
    \item ボタン名を保持する。
  \end{itemize}
  \item イベント要素
  \begin{itemize}
...
\hline
\ctmlabel{画面要素}
 & \ctmfig{0.7}{./images/Screen_st.png} \\
\hline
\ctmlabel{ボタン要素} & \ctmfig{0.6}{./images/Button_st.png} \\
\hline
\ctmlabel{イベント要素} & \ctmfig{0.6}{./images/Event_st.png} \\
\hline
\ctmlabel{分岐イベント要素} & \ctmfig{1.0}{./images/Sep_Event_st.png} \\
\hline
\ctmlabel{タイムアウト要素} & \ctmfig{1.0}{./images/Timeout_st.png} \\
\hline
\ctmlabel{遷移先のないイベント要素} & \ctmfig{0.6}{./images/Move_error_st.png} \\
\hline
\end{tabular}
...
\begin{figure}[tp]
  \centering
  \includegraphics[width=0.5\linewidth]{./images/EX_Dir_st.png}
...
また、各表示パターンとそれぞれの表示パターンを表示する条件の対応を表\ref{tab:Look_patterns}に、
...
\begin{figure}[tp]
  \centering
  \includegraphics[width=0.8\linewidth]{./images/SC_pt_st.png}
...
なお、イベントを追加する際、対象ボタンにすでに分岐イベントを定義している場合には、分岐イベントを追加できる。
...
\begin{figure}[tp]
  \centering
  \includegraphics[width=0.7\linewidth]{./images/Event_error_st.png}
...
図\ref{fig:pick}に示すCTMのボタン要素(1,2,3)の配置情報を示すJSONファイルの例を、リスト\ref{lst:JSON_example}に示す。ドラッグ操作の結果に応じて、配置情報を示すJSONファイルの内容を書き換える。
