...
本章では、本研究で新たに提案する、画面遷移を表現するためのダイアグラムである Condition Transition Map (CTM)、および、拡張した\tool の外観と機能について説明する。

\section{Condition Transition Map (CTM)}\label{sec:CTM}
本研究で新たに提案する、Condition Transition Map (CTM)は、画面遷移システムにおけるボタンとイベントの関係を表す。
CTMの例を図\ref{fig:ctm_example}に示す。
本研究では、ユーザによるCTMの編集を基に、
Markdown仕様、を生成した後、VDM\texttt{++}仕様を生成することで、
...
\begin{itemize}
  ...
  \item 分岐イベント要素
  \begin{itemize}
    \item 分岐イベント要素は、ある画面遷移システムにおいて、対象の有効ボタン押下時に条件付きで発生する動作を表し、その条件を、分岐条件として、菱形で描画する。
...
\begin{table}[tp]
  ...
  \begin{tabular}{|>{\centering\arraybackslash}m{5cm}|>{\centering\arraybackslash}m{6cm}|}
    ...
    \textbf{要素名} & \textbf{図} \\
    \hline
    画面要素 & \includegraphics[width=0.5\linewidth]{./images/画面.png} \\
    \hline
    ボタン要素 & \includegraphics[width=0.4\linewidth]{./images/ボタン.png} \\
    \hline
    イベント要素 & \includegraphics[width=0.4\linewidth]{./images/イベント.png} \\
    \hline
    分岐イベント要素 & \includegraphics[width=0.9\linewidth]{./images/条件分岐イベント.png} \\
    \hline
    タイムアウト要素 & \includegraphics[width=0.9\linewidth]{./images/タイムアウト.png} \\
    \hline
    遷移先のないイベント要素 & \includegraphics[width=0.4\linewidth]{./images/遷移エラー.png}\\
    \hline
  \end{tabular}
\end{table}
...
\begin{itemize}
  ...
  \item NoCode：「GUI操作によるVDM\texttt{++}仕様編集ページ」(\ref{sec:NoCodePage}節を参照)へ遷移する
\end{itemize}
...
図\ref{fig:initial_display}に示した「GUI操作によるVDM\texttt{++}仕様編集ページ」の外観に、説明のために領域ごとに枠を付与したものを、図\ref{fig:NoCodePageScreen}に示す。
...
CTM描画機能は、自然言語仕様記述ルール(\ref{sec:Specrule}節を参照)に則ったMarkdown仕様に
対応するCTMを、自動でCTM領域に描画する機能である。本機能は、ユーザがMarkdownファイルの選択、および、CTMの編集を行った際に、CTM領域にCTMを描画する。

CTM領域に表示するCTMの具体例は図\ref{fig:ctm_example}に示している。
...
フォルダツリー表示領域に表示するアイテムの対象は、フォルダ、および、拡張子が.mdのファイルである。
...
この機能により、ユーザによるプロジェクトフォルダ選択の有無、および、選択したMarkdown仕様ファイルに応じて、
実行可能な操作ボタンのみを操作ボタン領域に描画し、ユーザによる不適切な操作の実行を防止する。
本機能は、ユーザがフォルダツリー表示領域内のMarkdown仕様ファイルを選択した際に、本機能を実行する。
操作ボタンの表示パターンには、「GUI操作によるVDM\texttt{++}仕様編集ページ」への遷移直後、および、編集対象となっているVDM\texttt{++}仕様のクラスの種類に基づいて、
以下の4種類がある。
...
操作ボタンと各操作ボタン押下時処理の対応を、表\ref{tab:Look_patterns_operations}にそれぞれ示す。
...
表示パターンDは、Markdown仕様の種類が定まっていないままでのユーザによる、CTMの編集に関する操作実行を防止することを目的としている。
...
\begin{table}[tp]
  ...
  \begin{tabular}{|c|p{7.0cm}|}
    \hline
    \textbf{ボタン} & \textbf{各操作ボタン押下時処理} \\
    \hline
    フォルダの選択 &
...
その結果を\tool の記述ルールに則ったMarkdown仕様、および、
VDM\texttt{++}仕様に変換する機能である。

以降、各機能の詳細を示す。

\subsubsection{メニューバーの操作による仕様生成補助機能}
メニューバーの操作による仕様生成補助機能は、メニューバーに表示する、VDM\texttt{++}仕様生成に必要な、
ファイル、および、フォルダに関する機能を操作することで、VDM\texttt{++}仕様生成の補助を行う機能であり、メニューバーに表示する。
...
操作ボタン領域のボタン操作によるCTM編集機能は、操作ボタン領域のボタンを押すことでCTM領域に表示するCTMを編集する機能である。
また、本機能によりCTMを編集した直後、Markdown仕様、および、VDM\texttt{++}仕様を最新のCTMに対応したものに更新する。
...
入力した画面名に基づいて、
CTM上に新たな画面要素を生成し、
画面管理クラスのVDM\texttt{++}仕様にも対応する画面を自動で追加する。画面追加ダイアログの外観を図\ref{fig:screen_name_dialog_add}に、
...
入力したボタン名に基づいて、
CTM上に新たなボタン要素を生成し、対応するボタンを自動でVDM\texttt{++}仕様にも追加する。
ボタン追加ダイアログの外観を図\ref{fig:button_name_dialog}に、
「テストボタン」追加後の具体例を図\ref{fig:add_button}にそれぞれ示す。
\begin{figure}[tp]
  \centering
...
対応するタイムアウトを自動でVDM\texttt{++}仕様にも追加する。
...
タイムアウト後の動作を未定義としてVDM\texttt{++}仕様に追加する。タイムアウト設定時の入力エラーを、図\ref{fig:timeoutError}に示す。
...
対応するイベントを自動でVDM\texttt{++}仕様にも追加する。
...
単一イベントを追加する際、対象ボタンにすでに単一イベントを定義している場合には、
...
変更を行う際、画面管理クラスに編集対象の画面名を既に定義していた場合には、画面管理クラス内も変更後のクラス名(画面名)へと自動で変更を行う。
...
ユーザは、CTM領域上の要素を左クリックし削除対象とする要素を決定する。削除対象とする要素を決定後、ユーザが「削除」ボタンをクリックすることでCTM領域上、Markdown仕様上、および、VDM\texttt{++}仕様上から対応する要素、および、定義部分の削除を行う。
...
本機能により、ユーザは対象要素の種類に応じて、追加、編集、削除、および、コピーを効率的に実行できる。
...
複製の際、名称を定義するため要素の名称を入力するダイアログを表示する。コピー時ボタン名入力ダイアログの外観を図\ref{fig:copyB}に、
コピー時画面名入力ダイアログの外観を図\ref{fig:copyS}に、それぞれ示す。
