...
本ツールでは、CTM（\ref{sec:CTM}節を参照）上に表示する各CTM要素が保持するデータGUIElement（\ref{sec:ParsingComponent}節を参照）の中で座標データ、および、ノードサイズデータを保持するGUIElementをGUI要素と呼ぶ。
...
出力は、解析部、および、変換部へ渡すMarkdown文字列、変換済みVDM++文字列、および、GUI要素生成部へ渡すGUI配置情報（JSONファイル）である。
...
フォルダおよびファイル探索処理、ツリー表示用の内部データである\verb|ObservableCollection<FolderItem> FolderItems|を構築する処理である。
本処理は、フォルダ選択処理（\ref{sec:PM_SelectFolder}節を参照）から\verb|SelectedFolderPath|を入力として受け取る。そして、
\verb|SelectedFolderPath| 配下のフォルダ、および、ファイル群を走査し、Markdownファイルおよびフォルダ構造を取得する。
...
GUI要素の座標情報を読み込み、
GUI要素生成部（\ref{sec:GUIElementGenerationComponent}節を参照）の配置復元処理において配置情報を復元できるようにする。
\end{enumerate}
...
また、GUIElementが保持する条件分岐情報を表すBranchesのデータ構造を、表\ref{tb:EventBranch}に示す。

具体例に用いるMarkdown仕様を、コード\ref{lst:markdown_example}に示した具体例に対する処理後のGUIElement一覧を表\ref{tab:gui_element_example}に、Branchesを表\ref{tab:event_branch_example}に示す。
以降、処理について説明する。
\subsection{解析処理}\label{sec:ParsingProcess}

本処理は、Markdown仕様から、CTM要素である、画面要素、タイムアウト要素、ボタン要素、イベント要素、および条件分岐イベント要素を抽出し、
抽出したCTM要素を表\ref{tb:GUIElement}、\ref{tb:EventBranch}に示すGUIElementとして構造化し、GUIElementのリストである\verb|elements|を出力する処理である。
...
\begin{enumerate}
    ...
    画面クラスとして扱い、4.の画面クラス抽出、
    5.のタイムアウト抽出、6.のボタン抽出、7.のイベント抽出へ進む。
    ...
    画面クラスを解析する場合、
    先頭行（\verb|lines[0]|）の文字列を画面名として抽出する。
    ここでは、抽出した画面 \verb|GUIElement.Type| に \verb|Screen|、
    \verb|GUIElement.Name| に画面名を設定したGUIElementを生成し、\verb|elements| に追加する。
...
GUI要素の新たな配置情報をJSONファイルに記述し、更新したJSONファイルをポロジェクト管理部に返却する。
...
\begin{enumerate}
  \item GUI要素生成部
        ...
        GUI要素リストを取得する。
  \item 取得したGUI要素リスト、プロジェクト管理部より取得したMarkdown文字列を
