...
\texttt{Microsoft.\allowbreak Maui.\allowbreak Controls}名前空間(\ref{sec:NET_MAUI}説を参照) が提供する \texttt{ScrollView} コンポーネントを用いて
CTM表示用の表示枠を構成する。
...
\begin{enumerate}
  ...
  \begin{itemize}
      ...
      \begin{enumerate}[label=\roman*]
            ...
            \item コピー対象の\texttt{GUIElement}を複製し、ユーザ操作対応部が保持する\texttt{SelectedElement}のコピー用バッファ\texttt{CopiedElement}へ格納する。\texttt{CopiedElement}のデータ構造を表\ref{tab:copy-data}に示し、コピー処理を以下に示す。
...
Markdown仕様の記述ルール(\ref{sec:Specrule}節を参照)に含まない位置情報のみを保持する。CTM要素配置データの構造を、リスト\ref{lst:JSON_data}に示す。
