...
\begin{enumerate}
  ...
  \verb|SelectedFolderPath| をプロジェクト管理部が保持する。
...
\begin{enumerate}
  ...
  \item 画面索引辞書（\verb|screenIndex|）の設定
      ...
      発見した各 Markdown ファイルについて、画面名から当該ファイルを特定できるよう、
      画面索引辞書（\verb|screenIndex|）へ対応関係を登録する。
      登録にあたり、Markdown 文字列の先頭行を取得し、
      「\verb|# 画面一覧|」である場合は画面一覧クラスとして扱うため索引登録を行わない。
      一方で、先頭行が「\verb|##|」で始まる場合は画面クラスとして扱い、
      先頭行から抽出した画面名をキー、Markdown ファイルの絶対パスを値として登録する。
...
その内容を文字列として読み込み、表示パターンフラグの切替を行い、
...
\begin{enumerate}
  ...
  例えば、画面一覧を示す見出し（\verb|# 画面一覧|）である場合は
  画面一覧編集用の操作を表示するため、表示パターンフラグを表示パターンAに切り替える。  
  
  先頭行が画面定義を示す見出し（\verb|##|）で始まる場合は
  画面詳細編集用の操作を表示するため、表示パターンフラグを表示パターンBに切り替える。
  
  上記以外の場合は、初期状態としてクラス追加用の操作を表示するため、表示パターンフラグを表示パターンCに切り替える。
  本切替により、ユーザは現在編集中の仕様状態に応じた操作のみを選択可能となる。
...
画面切り替え処理は、ユーザ操作監視部から画面切り替え要求（\ref{sec:UOM_DoubleClick}節を参照）、および、切り替え先画面名を入力として受け取り、
...
\begin{enumerate}
  \item ユーザ操作監視部から画面要素のダブルクリック操作により、
        画面切り替え要求および切り替え先画面名を受け取る。

  \item プロジェクト管理部が保持している画面索引辞書（\verb|screenIndex|）を参照し、
        切り替え先画面名\verb|GUIElement.Name|に対応する Markdown ファイルパスを特定する。

  \item 対応する Markdown ファイルが存在する場合、
        対象ファイルを次の編集対象として設定する。

  \item 編集対象の変更に伴い、
        ファイル読込処理へファイル読込要求、および、対象Markdownファイルパスを出力し、
        Markdown 文字列および関連する GUI 要素を再構築する。
\end{enumerate}
...
\verb|elements| をプロジェクト管理部（\ref{sec:ProjectManagementComponent}節）から入力として受け取る。
...
出力は、ユーザ操作監視部（\ref{sec:UserOperationMonitoring}節）に渡すGUI要素リスト\verb|elements|である。
...
から受け取ったGUI要素リスト\verb|elements|をもとに、ツール上部に設置するボタンとCTM描画領域を描画、および、再描画する処理部である。
...
\subsection{初期描画処理}
初期描画処理は、GUI要素生成部から受け取ったGUI要素リスト\verb|elements|をもとに、
初期状態のCTMを生成する処理である。
...
\begin{enumerate}
    ...
    \item 各GUI要素が保持する座標情報をもとに、CTM描画領域上へCTM要素、および、ラベル文字列を描画する。
...
\begin{enumerate}
  ...
  \item 取得した表示パターンフラグをもとに、表示するボタンを決定する。
...
\begin{itemize}
      ...
      \item 上部操作ボタンのクリック操作監視処理（\ref{sec:UOM_OperationButtonClick}節）
...
\subsection{上部操作ボタンのクリック操作監視処理}\label{sec:UOM_TopBarClick}

上部操作ボタンのクリック操作監視処理は、
ツール上部に配置した操作ボタンのクリックを検知し、
対応する編集コマンドを確定して編集コマンド実行処理（\ref{sec:UOM_EditCommand}節を参照）へ通知する処理である。
本処理により、CTM描画領域上の右クリックメニューと同等の編集操作を、
...
CTM描画領域に対する座標入力を用いるヒットテスト処理（\ref{sec:UOM_HitTest}節を参照）は本処理では用いず、
...
\begin{enumerate}
  ...
  \begin{itemize}
      \item \textbf{画面追加}：
      \begin{enumerate}
            \item 画面追加用のポップアップを表示し、画面名を取得する。
            \item 既存画面名と重複する場合は追加しない
            \item 新しい画面要素（\verb|Type=Screen, Name=画面名|）をGUI要素リストに追加する。
            \item プロジェクト管理部へ更新要求を通知する（\ref{sec:FileUpdateProcess}節を参照）。
      \end{enumerate}
      \item \textbf{ボタン追加}：
      \begin{enumerate}
            \item ボタン追加用のポップアップを表示し、ボタン名を取得する。
            \item 既存ボタン名と重複する場合は追加しない
            \item 新しいボタン要素（\verb|Type=Button, Name=ボタン名|）をGUI要素リストに追加する。
            \item プロジェクト管理部へ更新要求を通知する（\ref{sec:FileUpdateProcess}節を参照）。
      \end{enumerate}
      \item \textbf{イベント追加}：
      \begin{enumerate}
            \item 対象ボタン選択用のポップアップを表示し、対象ボタン名を取得する。
            \item 対象のボタン要素に対して、すでにイベントが存在する場合は追加しない。
            \item 条件分岐イベント選択用のポップアップを表示し、単一イベントまたは条件分岐イベントを選択する。
            \begin{itemize}
                  \item 単一イベント選択時：
                  \begin{enumerate}
                        \item 単一イベント追加用のポップアップを表示し、イベント内容を取得する。
                        \item 新しいイベント要素（\verb|Type=Event|）を生成して追加する。
                  ...
                  \item 条件分岐イベント選択時：
                  \begin{enumerate}
                        \item 条件分岐イベント追加用のポップアップを表示し、
                        分岐条件文と遷移先イベントを取得する。
                        \item 新しい条件分岐イベント要素（\verb|Type=Event, Branches=null|）を生成して追加する。
      ...
      \item \textbf{タイムアウト追加}：
      \begin{enumerate}
            \item タイムアウト追加用のポップアップを表示し、タイムアウト内容を取得する。
            \item 新しいタイムアウト要素（\verb|Type=Timeout|）を生成して追加する。
      ...
      \item \textbf{削除}：
      \begin{enumerate}
            \item 対象GUI要素の名称を確認用ポップアップで表示し、削除確認を行う。
            \item ユーザが削除をキャンセルした場合は、処理を中断し、GUI要素リストを変更しない。
            \item ユーザが削除を確認した場合は、GUI要素リストから対象要素を除去する。
      ...
      \begin{enumerate}
            ...
            条件文をポップアップで表示し、編集用入力欄を提供する。
            \item ユーザがキャンセルした場合は、処理を中断し、GUI要素リストを変更しない。
            \item ユーザが編集を完了した場合、入力値を対象GUI要素に反映する。
