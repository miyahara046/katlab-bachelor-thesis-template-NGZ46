...
拡張部分のシステム構成を、図\ref{fig:system-architecture}に示す。
...
\begin{enumerate}
\item ユーザ操作監視部（\ref{sec:UserOperationMonitoringComponent}節を参照）によりユーザの保存操作を検出する。
...
本コンポーネントでは、Markdown形式仕様とVDM++形式仕様の変換に加え、GUI操作結果をMarkdown形式仕様へ反映する処理を担う。Markdown形式仕様からVDM++形式仕様への変換処理については、既存のツール\tool のMarkdown変換部（\ref(sec:Structure)小節を参照）を利用する。
...
次に各要素を
仕様記述として出力する順序を決定する。
出力規則は\tool のMarkdown仕様記述ルール（\ref{Specrule}小節を参照）に準拠しており、
...
本処理の流れを以下に示す。
\begin{enumerate}
    \item 現在のGUI要素リスト（\ref{sec:GUIElementGenerationComponent}節を参照）を走査する。
    \item GUI要素を、種類、および、関連関係に基づいて分類する。
    \item Markdown形式仕様記述ルールに基づき、出力順序を決定する。
    \item 各要素を対応するMarkdown形式仕様記述へ変換する。
    \item 変換したMarkdownテキストを連結し、最終的なMarkdown形式仕様文字列を生成する。
    \item 生成したMarkdownテキストをプロジェクト管理部へ通知し、ファイル更新処理（\ref{sec:FileUpdateProcess}節を参照）を要求する。
\end{enumerate}
...
\begin{enumerate}
    ...
    \item 遷移関係を表す要素については、GUI要素生成部で設定した接続情報を参照する。接続元および接続先ノードの外接矩形から矢印の開始点および終了点を算出し、矢印を描画する。
