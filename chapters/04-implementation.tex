...
拡張後の\tool は、既存の\tool 同様にMVVMアーキテクチャ(\ref{sec:MVVM}節を参照)を採用する。MVVMアーキテクチャ実装のために、本研究では
CommunityToolkit.Mvvmを使用する。
拡張後の\tool は、以下の6つの主要な処理部で構成する。
\begin{figure}[tp]
  \centering
  \includegraphics[width=1.0\linewidth]{./images/system-architecture.png}
...
\begin{itemize}
  ...
  ボタンのクリックイベントをトリガとして、Shellのルーティング機構を用いて、
  ...
  押下イベントをトリガとしてShellのルーティング機構を用いて、「スタートページ」へ遷移する。
\end{enumerate}
...
それの表示名である\texttt{Name}、フォルダかファイルかを判定する\texttt{IsFolder}と\texttt{IsFile}、ルートフォルダからどれだけ下の階層かを表す\texttt{Level}を保持する。
...
および種別情報\texttt{IsFolder}と\texttt{IsFile}に基づき、表示内容を構成する。
...
さらに、\texttt{ContentView}コンポーネントは、CTM領域上で発生するユーザ操作を検知するため、
...
操作対象のCTM要素を確定する処理である。ここで、CTM要素の編集(\ref{sec:UOM_EditCommand}節を参照)において、CTM要素をコピーして貼り付けた場合に、CTM領域上で、貼り付けたCTM要素が元のCTM要素と重なってしまう。そのため、CTM要素の一部が重なっており、複数のCTM要素がユーザのクリックによるポインタ座標に該当する場合は、描画順序(前面優先)に基づき最前面CTM要素を選択する。
...
\begin{enumerate}
  ...
  \item CTM要素の一部が重なっており、複数のCTM要素がユーザのクリックによるポインタ座標に該当する場合は、描画順序(前面優先)に基づき最前面CTM要素を選択する。
      ...
      \begin{enumerate}[label=\roman*]
            ...
            \begin{table}[tp]
                  ...
                  \hline
                      ...
                      \hline
                      X& float & コピー元のX座標(SelectedElement.X)\\
                      \hline
                      Y& float & コピー元のY座標(SelectedElement.Y)\\
                      \hline
                  \end{tabularx}
            \end{table}
            ...
            \begin{itemize}
                  ...
                  \begin{enumerate}[label=\alph*]
                        ...
                        \item \texttt{CopiedElement.X}に\texttt{SelectedElement.X}の値をコピーする。
                        \item \texttt{CopiedElement.Y}に\texttt{SelectedElement.Y}の値をコピーする。
                  \end{enumerate}
...
なお、配置する座標は、コピー元の要素の座標に定数値を加算することで決定する。そのため、CTM要素の重なりが発生する。
...
\begin{itemize}
      ...
      \begin{enumerate}
            ...
            \item \texttt{GUIElement.\allowbreak X}に貼り付け位置であるX座標を設定する。ここでX座標に$\texttt{CopiedElement.X}$(表\ref{tab:copy-data}を参照)を用いる。
            \item \texttt{GUIElement.\allowbreak Y}に貼り付け位置であるY座標を設定する。ここでY座標に$\texttt{CopiedElement.Y} + 30$(表\ref{tab:copy-data}を参照)を用いる。
      ...
      \begin{enumerate}
            ...
            \begin{itemize}
                  ...
                  \begin{enumerate}[label =\roman*]
                        ...
                        \begin{itemize}
                              ...
                              \item \texttt{GUIElement.\allowbreak Target}に\texttt{null}を設定する。
                              \item \texttt{GUIElement.\allowbreak X}に貼り付け位置であるX座標を設定する。ここでX座標に$\texttt{CopiedElement.X} + 240$を用いる。
                              \item \texttt{GUIElement.\allowbreak Y}に貼り付け位置であるY座標を設定する。ここでY座標に$\texttt{CopiedElement.Y} + 30$を用いる。
                              \item \texttt{GUIElement.\allowbreak IsMovable}に\texttt{true}を設定する。
                              \item \texttt{GUIElement.\allowbreak IsSelected}に\texttt{false}を設定する。
            ...
            \end{itemize}
                  ...
                  \begin{itemize}
                        ...
                        \item \texttt{GUIElement.X}に貼り付け位置であるX座標を設定する。ここでX座標に$\texttt{CopiedElement.X}$を用いる。
                        \item \texttt{GUIElement.Y}に貼り付け位置であるY座標を設定する。ここでY座標に$\texttt{CopiedElement.Y} + 30$を用いる。
...
CTM要素配置データファイルは、CTM要素配置データの情報を保持するJSON形式のファイルであり、拡張子として .positions.jsonを持つ。
\begin{figure}[tp]
...
\begin{enumerate}
  ...
  CTM要素配置データファイル(拡張子 .positions.json)が存在するかどうかを確認する。
...
\begin{enumerate}
  ...
  CTM要素配置データファイル(拡張子 .positions.json)へ書き込み、保存する。
  同名のファイルが存在しない場合は、Markdown仕様ファイルと同名の CTM要素配置データファイル(拡張子 .positions.json)を
  新規作成する。
            ...
            \begin{enumerate}[label=\roman*.]
                  ...
                  \begin{enumerate}[label=\alph*.]
                       ...
                       4.B.ii.の次行以降を走査し、行頭が「\texttt{\#}」から始まるかどうかを確認する。
                        ...
                        \texttt{BulletPattern}によって抽出した箇条書き本文を、ボタン名として抽出する。
                  ...
                  \begin{enumerate}[label=\alph*.]
                        ...
                        なお、以下のいずれかを満たす行に到達した場合を、終了行と判定する。
                        \begin{itemize}
                              \item 見出し行(「\texttt{\#\#\#}」で始まる行)に到達する。
                              \item ファイル末尾に到達する。
                        \end{itemize}
                        ...
                        23行目の「\texttt{表示部に1 が入力されていない → 画面F へ}」までを、それぞれ抽出する。
                        ...
                        (\texttt{→})以降の文字列をイベント内容\texttt{eventContent}として、それぞれ抽出する。
