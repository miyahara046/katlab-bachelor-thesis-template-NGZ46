...
View 層は、GUIの表示、および、ユーザ入力の受付を担っており、描画部はツールの外観の整形、ページの切り替え、ユーザ入力の受付、CTMの可視化を行う。
ViewModel 層は、View 層と Model 層間のデータバインディングを担当しており、
ユーザ操作対応部は、ユーザ入力を、それぞれの操作について判断し、
Model 層の各処理部へ処理を指示する。
Model 層は、プロジェクト管理部、解析部、GUI要素生成部、変換部から構成し、
プロジェクトフォルダの管理、Markdown仕様の解析、CTMからMarkdown仕様への変換、および、Markdown仕様からVDM++仕様への変換を担う。
...
描画部は、ユーザの入力の受付、および、ユーザ操作対応部(\ref{sec:UserOperationMonitoring}節を参照)から参照するデータをもとに、表示ページの切り替え、操作ボタン領域に設置するボタン、フォルダツリー表示領域に表示するフォルダツリー、CTMの表示をする処理部である。

本処理部は、.NET MAUIの標準ライブラリであるMicrosoft.Maui.Controls(\ref{sec:NET_MAUI}節を参照)を用いて実装している。
...
「スタートページ」は、本研究の拡張で新たに追加した、ツール起動時に最初に表示する画面であり、
「Markdown仕様記述ページ」、
...
へ遷移する2つのボタンのみを配置している。

「Markdown仕様記述ページ」、および、「GUI操作によるVDM\texttt{++}仕様編集ページ」には、
本処理を実行するための「スタートページへ戻る」ボタンを設置している。
...
本処理は、
ユーザ操作対応部が保持する現在の表示パターンを決定する変数である表示パターンフラグを参照し、表示パターンフラグに基づいて
...
\begin{enumerate}
  \item ユーザ操作対応部が管理する現在の表示パターンフラグを参照。
...
フォルダツリー描画処理は、フォルダツリー表示領域にユーザ操作対応部が保持する、フォルダツリーの各行を表すデータ構造である\verb|FolderItem|
...
\begin{enumerate}
  ...
  検知した\verb|FolderItem|をユーザ操作対応部へ出力する。
...
\verb|VdmContent| をVDM\texttt{++}表示領域に描画する処理である。
...
また、.NET MAUIのデータバインディング機構を用いて表示の更新をおこなう。
...
メニューバー描画処理は、ページ上部のメニューバーに、フォルダ選択、および、新規ファイル作成の操作項目を配置し、
ユーザ操作対応部(\ref{sec:UserOperationMonitoring}節を参照)へ編集コマンドを出力する処理である。

本処理では、.NET MAUI(\ref{sec:NET_MAUI}節を参照)が提供する\verb|Microsoft.Maui.Controls|のメニューバー関連コンポーネントを用いる。
...
フォルダ選択、および、新規ファイル作成をメニューバーに配置する。

\item コマンドの関連付け

フォルダ選択に編集コマンドのフォルダ選択、および、新規ファイル作成に編集コマンドの新規ファイル作成を、それぞれ結び付ける。

\item メニューバー操作の検知

\verb|Microsoft.Maui.Controls|によってユーザのメニューバー項目選択を検知した場合、当該項目に割り当てたコマンドを、ユーザ操作対応部へ出力する。

\end{enumerate}

\subsection{CTM領域描画処理}\label{sec:CTMArea}
CTM領域(\ref{sec:CTM}節を参照)描画処理は、CTMを表示するための表示枠を構成し、
CTM生成処理(\ref{sec:CreateCTM}節を参照)で実装した描画ロジックを接続するとともに、
...
CTM領域に描画機能および入力受付機能を結び付ける。
この要素は、
CTM描画処理で定義した描画ロジックを呼び出す役割を持つ。

また、CTMの規模が増加した場合に備えて、
CTM描画処理の結果に基づき、
...
\begin{enumerate}
  ...
  本処理では、\verb|Microsoft.Maui.Controls| が提供するジェスチャ認識、
...
\item 右クリック操作
  ...
  その後、右クリック位置の座標と右クリックであることをユーザ操作対応部へ通知する。
  \end{itemize}
...
CTM生成処理は、ユーザ操作対応部が保持する、CTM領域に表示する各CTM要素のデータを格納した、
...
\begin{enumerate}
      ...
      描画したすべてのノードおよび分岐条件について、
      \verb|BRectangle|を記録し、\verb|HitRegions|を作成し、ユーザ操作対応部に渡す。
...
コンテキストメニュー描画処理は、ユーザ操作対応部より、コンテキストメニュー描画要求、対象のCTM種別、および、分岐条件かどうかを受け取り、
コンテキストメニューを描画する。
...
表示する項目は、右クリック位置の操作対象CTM種別に応じて切り替える。
...
\begin{enumerate}
      ...
      ユーザ操作監視部からコンテキストメニュー描画要求、CTM種別、および、分岐条件かどうかを受け取る。

  
      \item メニュー項目の決定

      ユーザから受け取った操作対象の種別、および
      分岐条件かどうかに基づき、
      表示するメニュー項目の集合を決定する。
...
表示後、ユーザによる選択、および、入力を、ユーザ操作対応部へ返す。
...
\begin{enumerate}
  ...
  ユーザ操作対応部からのダイアログ描画要求、および、編集コマンドに応じたダイアログを受け取る。
  
  \item ダイアログの表示

  ユーザ操作対応部から受け取った編集コマンドに応じたダイアログを表示する。
  表示するダイアログは以下の4種類のダイアログに分類できる。
  \begin{itemize}
      \item 通知用ダイアログ
...
入力は左クリック座標、出力は対象CTM要素の選択状態の更新、および、CTM描画要求である。

本処理の流れを、以下に示す。
\begin{enumerate}
  \item 描画部から左クリック動作であること、および、ポインタ座標 $(x, y)$ を取得する。
  \item ポインタ座標 $(x, y)$ をユーザ操作対応部で保持する状態変数X\_s、Y\_sとして保持する。
  \item ユーザ操作対応部が保持する操作対象CTM要素を表す状態変数\verb|SelectedElement|にすでにCTM要素が格納されている場合
  ...
  \item 描画部へCTM描画要求を発行し、強調表示状態を更新する。
...
式\ref{eq:hit}により判定する。式中のX、Y、W、H、は表\ref{tab:hit_test_rect}にそれぞれ示す、属性と等しい。
...
右クリック操作対応処理は、描画部による左クリックによる座標を受け取り、
...
\begin{enumerate}
  ...
  以降、描画部からドラッグ終了を受け取るまで繰り返す。
  \begin{enumerate}[label=\roman*.]
  
      \item ポインタ移動イベントごとに、押下開始座標(X\_s,Y\_s)に移動量を加算していく。
...
\begin{enumerate}
  ...
  \item 選択対象を \verb|SelectedItem| として保持する。
      ...
      \item 選択対象がファイル\verb|IsFile == true|である場合は、
        プロジェクト管理部へファイル読込要求を出力し、編集対象ファイルを切り替える。
...
要素の追加、更新、削除を行った上で、更新後の\verb|elements|、描画部へのCTM描画要求、
...
\begin{enumerate}
  ...
  \begin{itemize}
      \item \textbf{画面追加}：
      \begin{enumerate}
            \item 画面追加用のダイアログ描画要求を出力し、入力を受け取る
      ...
      \item \textbf{ボタン追加}：
      \begin{enumerate}
            \item ボタン追加用のダイアログ描画要求を出力し、入力を受け取る
      ...
      \item \textbf{イベント追加}：
      \begin{enumerate}
            \item 対象ボタン選択用のダイアログ描画要求を出力し、入力を受け取る
            \item 対象のボタン要素に対して、すでにイベントが存在する場合は、エラー表示用のダイアログ描画要求を出力し、追加しない。
            \item 分岐イベント選択用のダイアログ描画要求を出力し、入力を受け取り、単一イベント、または、分岐イベントを選択する。
            \begin{itemize}
                  \item 単一イベント選択時：
                  \begin{enumerate}
                        \item 単一イベント追加用のダイアログ描画要求を出力し、入力を受け取り、イベント内容を取得する。
                  ...
                  \item 分岐イベント選択時：
                  \begin{enumerate}
                        \item 分岐イベント追加用のダイアログ描画要求を出力し、入力を受け取り、
                        分岐条件文と遷移先イベントを取得する。
      ...
      \item \textbf{タイムアウト追加}：
      \begin{enumerate}
            \item タイムアウト追加用のダイアログ描画要求を出力し、入力を受け取り、タイムアウト内容を取得する。
      ...
      \item \textbf{削除}：
      \begin{enumerate}
            \item 対象GUI要素の名称確認用のダイアログ描画要求を出力し、削除確認を行う。
      ...
      \begin{enumerate}
            ...
            \item 対象CTM要素の種別に応じて、編集用のダイアログ描画要求を出力し、入力を受け取る。
      ...
      \item \textbf{新規ファイル作成}：
      \begin{enumerate}
            \item ファイル名入力用のダイアログ描画要求を出力し、入力を受け取る。
            \item 取得したファイル名が\verb|FolderItems|内に存在しないか探索する。
            \item \verb|FolderItems|内に同一のファイル名が存在する場合エラー表示用のダイアログ描画要求を出力し、処理を中断する。
      ...
      \begin{enumerate}
            \item クラス名(画面名)入力用のダイアログ描画要求を出力し、入力を受け取る。
...
本処理部の入力は、ユーザ操作対応部(\ref{sec:UserOperationMonitoring}節を参照)による要求や\verb|elements|、および、プロジェクトフォルダである。
...
\begin{enumerate}
  ...
  ユーザ操作対応部、および、画面切り替え処理からのファイル選択要求を受け取り、
  ...
  \item GUI要素生成部の呼び出し
      ...
      CTM要素のサイズ、配置情報を設定した上で、\verb|elements|をプロジェクト管理部へ返す。
...
ファイル更新処理は、
ファイル更新要求をユーザ操作対応部から受け取り、ユーザ操作対応部が保持する\verb|elements|を参照し、
変換部から受け取った
...
CTM配置情報(JSON)が存在する場合は、
\verb|GUIElement.Name| をキーとして復元し、
配置情報が存在しない場合、または、当該要素の座標が存在しない場合は、
要素種別に応じた初期配置に基づいて座標を決定する。

初期配置では、要素を種別ごとに縦方向へ積み上げて配置する。
このとき、種別 $t$ ごとに 0 始まりの連番 $i_t$ を保持し、
同一種別内の出現順に $i_t$ を増加させる。

横方向の座標 $X$ は、種別ごとに固定した列位置を用いる。
縦方向の座標 $Y$ は、基準座標 $Y_0$ と、縦方向間隔 $d$ を用いて決定する。、
ここで、$Y_0$ は上端余白、$d$ は要素間の縦方向間隔である。
実際には、左列のX座標を $X_{\mathrm{left}}=40\mathrm{px}$、
通常ノード幅を $W_{\mathrm{node}}=160\mathrm{px}$、
列間隔を $S_{\mathrm{col}}=80\mathrm{px}$ として定義し、
イベント列のX座標を $X_{\mathrm{mid}}$ として次式で算出する。
また、$Y_0=40\mathrm{px}$、$d=80\mathrm{px}$ とする。

式\ref{eq:initial_position_x_by_type}で$X$ 座標、\ref{eq:initial_position_y_by_type}で$Y$座標を決定する。
$X_{\mathrm{fix}}$ 、および、$Y_{\mathrm{fix}}$は固定値である。

\begin{equation}\label{eq:initial_column_mid}
X_{\mathrm{mid}} = X_{\mathrm{left}} + W_{\mathrm{node}} + S_{\mathrm{col}}
\end{equation}

\begin{equation}\label{eq:initial_position_x_by_type}
X(e) =
\begin{cases}
X_{\mathrm{left}} & (\verb|Type|=\verb|Screen|) \\
X_{\mathrm{left}} & (\verb|Type|=\verb|Button|) \\
X_{\mathrm{mid}}  & (\verb|Type|=\verb|Event| \land \verb|Branches|=\verb|null|) \\
X_{\mathrm{fix}}  & (\verb|Type|=\verb|Timeout|) \\
\end{cases}
\end{equation}

\begin{equation}\label{eq:initial_position_y_by_type}
Y(e) =
\begin{cases}
Y_0 + i_{\mathrm{screen}} \cdot d & (\verb|Type|=\verb|Screen|) \\
Y_0 + i_{\mathrm{button}} \cdot d & (\verb|Type|=\verb|Button|) \\
Y_0 + i_{\mathrm{event}} \cdot d  & (\verb|Type|=\verb|Event| \land \verb|Branches|=\verb|null|) \\
Y_{\mathrm{fix}}                  & (\verb|Type|=\verb|Timeout|) \\
\end{cases}
\end{equation}
...
\begin{enumerate}
      ...
      \begin{enumerate}[label=\roman*.]
            ...
            \item 座標を決定する。
                  ...
                  存在しない場合は式\ref{eq:initial_position_x_by_type}および式\ref{eq:initial_position_y_by_type}により初期配置座標を算出して設定する。
