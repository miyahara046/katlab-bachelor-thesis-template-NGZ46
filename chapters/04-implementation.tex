...
なお、本研究の拡張はWindowsOSにのみ対応しており、それ以外のOSには対応していない。
以下に実装の詳細について説明する。

\section{スタートページの実装}
本節では、本研究の拡張として新たに追加した「スタートページ」の実装について説明する。
「スタートページ」は、ツール起動時に最初に表示する画面であり、
「Markdown仕様記述ページ」、
...
\begin{enumerate}
  ...
  遷移先ページはContentPageとして生成する

  \item ユーザがスタートページ上の「NoCode」ボタンを押下した場合も同様に、
  ボタンのクリックイベントをトリガとして
  Shellのナビゲーション機構を用いて、
  ...
  ユーザが「スタートページに戻る」ボタンを押下した場合、
  押下イベントをトリガとしてShellのナビゲーション機構を用いて、「スタートページ」のルートへ遷移し、
  再び「スタートページ」上で遷移先を選択できる状態とする。
\end{enumerate}

\section{GUI操作によるVDM\texttt{++}仕様編集ページの構成}
以降の節では、「GUI操作によるVDM\texttt{++}仕様編集ページ」について説明する。
本研究の拡張で追加した部分も既存の\tool 同様にMVVMアーキテクチャ(\ref{sec:MVVM}節を参照)を採用している。MVVMアーキテクチャ実装のために、本研究では
CommunityToolkit.Mvvm(\ref{sec:CommunityToolkit.Mvvm}節を参照)を使用している。拡張した\tool のシステム構成を、図\ref{fig:system-architecture}に示す。
...
ユーザ操作監視部は、ユーザ入力を、それぞれの操作について判断し、
Model 層の各処理部へ処理を指示する。
View 層は、GUIの表示、および、ユーザ入力の受付を担っており、描画部はCTMの可視化、ツールの外観の整形をする。
Model 層は、プロジェクト管理部、解析部、GUI要素生成部、変換部から構成し、
プロジェクトフォルダの管理、Markdown仕様の解析、および、Markdown仕様からVDM++仕様への変換を担う。
...
ユーザ操作監視部は、\tool 上で発生するユーザによる操作を監視し、
操作対象の要素を特定した上で、プロジェクト管理部(\ref{sec:ProjectManagementComponent}節を参照)、および、
描画部(\ref{sec:DrawingComponent}節を参照)へ各要求を発行する統括処理部である。
本処理部は、MVVMにおけるView層Model層間の橋渡しとして振る舞い、
プロジェクト管理部へ、フォルダ選択要求、
ファイル選択要求、ファイル更新要求、画面切り替え要求、および、ファイル作成要求を行い、
描画部へ初期描画要求、および、再描画要求を行う。

本処理部の入力は、 ユーザによる操作であるクリック、および、ドラッグ、
CTM(\ref{sec:CTM}節を参照)領域に表示する各CTM要素のデータを格納した、\verb|GUIElement|のリストである、\verb|elements|である。
ここで、\verb|GUIElement| は、各CTM 要素を構成する構造データである。GUIElementのデータ構造を、表\ref{tb:GUIElement}に示す。
GUIElementでは、各CTM要素の種別を\verb|Type|属性として保持する。\verb|Type|属性は列挙型である\verb|GuiElementType|型のデータである。
...
\end{itemize}
また、GUIElementはCTM要素の名称を\verb|Name|属性として、CTM要素のターゲット情報を\verb|Target|属性として、CTM要素のX座標を\verb|X|属性として、
CTM要素のY座標を\verb|Y|属性として、CTM要素の横幅を\verb|Width|属性として、CTM要素の立幅を\verb|Height|属性として、ユーザが選択しているかどうかを\verb|IsSelected|属性として、
CTM要素がドラッグ操作によって移動可能かどうかを\verb|IsMovable|属性として、
分岐イベント要素のデータリストを\verb|Branches|属性として、
CTM要素が分岐イベント要素であるかどうかを\verb|IsBranch|属性として保持する
...
\hline
IsSelected & bool & ユーザが選択しているかどうか \\
...
出力は、フォルダ選択要求、
ファイル選択要求、ファイル更新要求、画面切り替え要求、ファイル作成要求、初期描画要求、および、再描画要求である。

本処理部で用いるユーザによる操作のポインタ座標は、.NET MAUI (\ref{sec:NET_MAUI}節を参照)の座標系に基づいており、
CTM領域の左上を原点とする座標系である。

本処理部の主要な処理は、以下の7つである。
\begin{itemize}
      \item ヒットテスト処理(\ref{sec:UOM_HitTest}節を参照)
      \item 左クリック操作監視処理(\ref{sec:UOM_Click}節を参照)
      \item 右クリック操作監視処理(\ref{sec:UOM_Context}節を参照)
      \item 左ダブルクリック操作監視処理(\ref{sec:UOM_DoubleClick}節を参照)
      \item ドラッグ移動操作監視処理(\ref{sec:UOM_Drag}節を参照)
      \item 操作ボタンのクリック操作監視処理(\ref{sec:UOM_TopBarClick}節を参照)
...
本処理は、描画部が保持する各CTM要素が画面上に占める領域を表す表示属性
である、外接矩形を用いて、操作座標が外接矩形内に存在するかを判定する。
本研究における外接矩形は、各CTM要素に対応付けた表示領域を表すデータ構造であり、
ヒットテストの結果として操作対象となるCTM要素を特定するため、
当該GUIElementへの参照を属性として保持する。外接矩形のデータ構造を表\ref{tab:hit_test_rect}に示す
...
式\ref{eq:hit}により判定する。式中のX、Y、W、H、は表\ref{tab:hit_test_rect}にそれぞれ示す、属性のことである。
...
\begin{enumerate}
  ...
  \item 描画部から、外接矩形を格納したリストである外接矩形リストを取得する。   
  \item 概説矩形リストを先頭から走査し、式\ref{eq:hit}により包含判定を行う。
  \item 複数のGUI要素がヒットする場合は、描画順序(前面優先)に基づき最前面GUI要素を採用する。
      描画順序は、外接矩形の \verb|ZIndex| 属性を参照し、値が大きいほど前面に描画していると判定する。
  \item 採用したCTM要素をユーザ操作監視部で保持する、現在ユーザが選択しているCTM要素\verb|SelectedElement|
  として確定する。
...
入力は左クリック座標、出力は対象CTM要素の選択状態の更新、および、再描画要求である。

本処理の流れを、以下に示す。
\begin{enumerate}
  \item CTM描画領域として使用している \verb|GraphicsView|(\ref{sec:NET_MAUI}節を参照) の
      \verb|StartInteraction| イベントで押下操作を検知し、
      イベント引数 \verb|TouchEventArgs| から操作座標 $(x, y)$ を取得する。
  \item ヒットテスト処理(\ref{sec:UOM_HitTest}節を参照)へ、クリック座標を渡し、\verb|SelectedElement| を確定する。
  \item 既存の選択CTM要素が存在する場合、当該CTM要素の \verb|GUIElement.IsSelected| を \verb|false| に更新する。
  \item \verb|SelectedElement| の \verb|GUIElement.IsSelected| を \verb|true| に更新する。
  \item 描画部へ再描画要求を発行し、強調表示状態を更新する。強調表示状態は、
        選択中GUI要素の周囲をオレンジ色の太線で囲うことで表現する。
  \item 左ダブルクリック操作監視処理(\ref{sec:UOM_DoubleClick}節を参照)へ、左ダブルクリックの判定を要求する。
  \item ドラッグ移動操作監視処理(\ref{sec:UOM_Drag}節を参照)へ、ドラッグの判定を要求する。
\end{enumerate}

\subsection{右クリック操作監視処理}\label{sec:UOM_Context}
右クリック操作監視処理は、右クリックによる編集要求を検出し、
対象CTMの種類(\verb|GUIElement.Type==Screen、Button、Event、Timeout|)および条件分岐(\verb|GUIElement.IsBranch == true|)に基づき、
表示するコンテキストメニューを決定し、コンテキストメニュー内のコマンドの選択結果を編集コマンドとして
発行する処理である。
本処理の入力は、右クリック座標と現在のGUI要素リスト、出力は編集コマンドである。

本処理の流れを以下に示す。
\begin{enumerate}
  \item 右クリック座標 $(x, y)$ を\verb|WinUI|(\ref{sec:WinUI}節を参照)の\verb|PointerPressed|イベントから受け取る。
  \item ヒットテスト処理(\ref{sec:UOM_HitTest}節を参照)へ、クリック座標を渡し、\verb|SelectedElement|を確定する。
  \item 対象CTM要素の 種類\verb|GUIElement.Type| に応じてコンテキストメニューに表示するコマンドを決定する。
        例えばボタン要素には「イベント追加」、「コピー」、「貼り付け」、「ボタン名変更」、「削除」を表示し、イベント要素には「イベント変更」、「削除」を表示する。
  \item 分岐イベントの場合は、分岐条件ごとに右クリックを区別する。
        これにより「分岐編集」、「分岐削除」を対象分岐条件に紐付けて実行できる。
  \item ユーザが選択したコマンドを編集コマンドとして編集コマンド実行処理(\ref{sec:UOM_EditCommand}節を参照)へ、通知する。
\end{enumerate}
...
本処理の流れを以下に示す。
\begin{enumerate}
  \item 左クリック操作監視処理(\ref{sec:UOM_Click})から判定要求を受け取り、\verb|SelectedElement|を参照する。
  ...
  \item 画面要素であった場合、現在時刻を\verb|DateTime.UtcNow|より取得し、
        直前クリック対象要素が同一CTM要素であること、および、
        直前クリック自国との時刻差分が閾値以内であることを同時に満たすか判定する。
      \begin{itemize}
...
ドラッグ操作監視処理は、CTM 図上に配置された CTM 要素に対するドラッグ操作を検出し、
当該 CTM 要素の配置変更を反映する処理である。
入力は、押下開始座標、ポインタ移動量、および、解放座標であり、
出力は、更新後の GUI 要素座標情報、および、必要に応じたファイル更新要求である。
...
本処理の流れを、以下に示す。
\begin{enumerate}
  \item 左クリック操作監視処理(\ref{sec:UOM_Click}節を参照)により、
  押下開始座標 $(x_s, y_s)$ を受け取り、ヒットテスト処理(\ref{sec:UOM_HitTest}節を参照)によって、
  操作対象となる CTM 要素を確定する。
  \item 対象 CTM 要素の \verb|GUIElement.IsMovable| を参照し、移
  動不可(\verb|IsMovable == false|)である場合は、ドラッグ操作を開始せず、処理を終了する。
  \item ドラッグ開始時点における CTM 要素の座標 $(X, Y)$ と、
  押下開始座標 $(x_s, y_s)$ との差分を、オフセット量 $(\Delta x, \Delta y)$ として保持する。
  \item ポインタ移動イベントごとに、現在のポインタ座標 $(x, y)$ から、
  オフセット量 $(\Delta x, \Delta y)$ を差し引くことで、
  新しい CTM 要素座標 $(X', Y')$ を算出し、\verb|GUIElement.X| および \verb|GUIElement.Y| を更新する。
  \item 座標更新のたびに、描画部(\ref{sec:DrawingComponent}節を参照)へ、
  再描画要求を発行し、CTM 図上に移動結果を即時反映する。
  \item ポインタ解放時に、最終的な CTM 要素座標を確定する。プロジェクト管理部のファイル更新処理(\ref{sec:FileUpdateProcess}節を参照)を要求する。
\end{enumerate}
...
操作ボタンのクリック操作監視処理は、
操作ボタン領域に配置した操作ボタンのクリックを検知し、
対応する編集コマンドを確定して編集コマンド実行処理(\ref{sec:UOM_EditCommand}節を参照)へ通知する処理である。
本処理により、CTM領域上の右クリックによるコンテキストメニューと同等の編集操作を、操作ボタンからも実行可能とする。

本処理の入力は、操作ボタンのクリックイベント、および、操作ボタン領域の操作ボタンに対応付けたコマンドである。
出力は、確定した編集コマンド、および、必要に応じて操作対象要素\verb|SelectedElement|を含む編集要求である。
なお、操作ボタン
CTM領域に対する座標入力を用いるヒットテスト処理(\ref{sec:UOM_HitTest}節を参照)を用いず、
.NET MAUI (\ref{sec:NET_MAUI}節を参照)が提供する標準のUI イベント機構であるボタンのクリックイベント、および 、Command バインディングを用いて実装している。

本処理の流れを、以下に示す。
\begin{enumerate}
  \item 操作ボタンのクリックイベントを受け取る。
  \item 現在の選択要素 \verb|SelectedElement| を参照し、
        コマンドが選択要素の編集を必要とするか判定する。
  \item 選択要素の編集を必要とするコマンドであり、
        \verb|SelectedElement| が \verb|null| である場合は、
        エラー表示を行い、以降の処理を中断する。
  \item 選択要素の編集を必要とするコマンドであり、\verb|SelectedElement| が \verb|null| でない場合は、
  コマンドを、編集コマンド実行処理(\ref{sec:UOM_EditCommand}節を参照)へ通知する。
  \item 操作対象要素を必要としないコマンドの場合は、
  コマンドを、編集コマンド実行処理(\ref{sec:UOM_EditCommand}節を参照)へ通知する。
\end{enumerate}

\subsection{フォルダツリー操作監視処理}\label{sec:UOM_FolderTree}
フォルダツリー操作監視処理は、フォルダツリー表示領域に対するクリック操作を監視し、
フォルダの展開、および、折りたたみ、
編集対象ファイルの切替を行う処理である。
本処理は、フォルダツリーの各行を表すデータ構造である\verb|FolderItem|を介して、
プロジェクト管理部へファイル読込要求
  (\ref{sec:PM_FileLoadProcess}節を参照)、ファイル更新要求(\ref{sec:FileUpdateProcess}節を参照)、および、描画部へ初期描画要求(\ref{sec:first-draw}節を参照)、フォルダ描画要求(\ref{sec:FolderTree}節を参照)
  を出力する処理である。
ここで、\verb|FolderItem|のデータ構造を表\ref{tb:FolderItem}に示す。
フォルダツリーのデータは、\verb|FolderItem|のコレクションである \verb|FolderItems| として保持する。
\verb|FolderItem| は、フォルダ、または、ファイルの絶対パスである\verb|FullPath| を持ち、
フォルダかファイルかを判定する\verb|IsFolder|、\verb|IsFile|。
さらに、子要素の表示、または、非表示を制御する\verb|IsExpanded| と \verb|IsVisible| を保持する。
\begin{table}[tp]
\centering
...
本処理の入力は、ユーザによる、フォルダツリー行の選択であり、
出力は、プロジェクト管理部へファイル読込要求
  (\ref{sec:PM_FileLoadProcess}節を参照)、ファイル更新要求(\ref{sec:FileUpdateProcess}節を参照)、および、
  描画部へ初期描画要求
  (\ref{sec:first-draw}節を参照)、および、フォルダ描画要求(\ref{sec:FolderTree}節を参照)を通知する。

本処理の流れを、以下に示す。
\begin{enumerate}
  \item ユーザによるフォルダツリー行の選択イベントを受け取る。
  \item 選択対象を \verb|selectedItem| として保持する。
      \begin{itemize}
  
      \item 選択対象がファイル\verb|IsFile == true|である場合は、
        プロジェクト管理部へファイル読込要求、および、ファイル更新要求を通知し、編集対象ファイルを切り替える。
        切り替えた際、プロジェクト管理部から\verb|elements|を受け取り、描画部へ、初期描画要求を通知する。
  
      \item 選択対象がフォルダ\verb|item.IsFolder == true|である場合は、フォルダの展開、および、折りたたみを行う
      その際、\verb|IsExpanded|を反転し、配下の\verb|FolderItems|について\verb|IsVisible| を反転する。
      これにより、展開時は子要素を表示し、折りたたみ時は子要素を非表示とする。更新後、描画部へフォルダ描画要求を通知する。
      \end{itemize}
\end{enumerate}
...
要素の追加、更新、削除を行った上で、更新後の\verb|elements|、描画部への再描画要求、
プロジェクト管理部へのファイル更新要求を出力する。
...
\begin{enumerate}
  ...
  \item 編集コマンドの種類に応じて、必要な入力項目を決定し、ダイアログを表示する。
  例えば、ボタン追加ではボタン名、イベント追加では「対象ボタン」「イベント内容の選択」「イベント」を要求する。
  条件分岐追加では「分岐条件」と「分岐先イベント」を要求する。
  \begin{itemize}
  ...
  \begin{itemize}
      \item \textbf{画面追加}：
      \begin{enumerate}
            \item 画面追加用のダイアログを表示し、画面要素名を取得する。画面名追加用のダイアログは
            .NET MAUI(\ref{sec:NET_MAUI}節を参照)標準ライブラリ内の\verb|DisplayAlert|を使用し、入力欄が一つのダイアログを表示する。
      ...
      \begin{enumerate}
            ...
            ボタン名追加用のダイアログは.NET MAUI標準ライブラリ内の\verb|DisplayPrompt|を使用し、入力欄が一つのダイアログを表示する。
      ...
      \begin{enumerate}
            ...
            対象ボタン選択用のダイアログは.NET MAUI標準ライブラリ内の\verb|DisplayActionSheet|を使用し、\verb|elements|内のボタン名を列挙し、選択用ダイアログを表示する。
            \item 対象のボタン要素に対して、すでにイベントが存在する場合は追加しない。
            \item 分岐イベント選択用のダイアログを表示し、単一イベントまたは分岐イベントを選択する。
            \begin{itemize}
                  \item 単一イベント選択時：
                  \begin{enumerate}
                        \item 単一イベント追加用のダイアログを表示し、イベント内容を取得する。単一イベント追加用のダイアログはCommunityToolkit.Maui(\ref{sec:CommunityToolkit.Maui}節を参照)の\verb|Popup| を用いて表示する。
                  ...
                  \item 分岐イベント選択時：
                  \begin{enumerate}
                        \item 分岐イベント追加用のダイアログを表示し、
                        分岐条件文と遷移先イベントを取得する。分岐イベント追加用のダイアログはCommunityToolkit.Mauiの\verb|Popup| を用いて表示する。
      ...
      \begin{enumerate}
            ...
            タイムアウト追加用はCommunityToolkit.Mauiの\verb|Popup| を用いて表示する。
      ...
      \begin{enumerate}
            ...
            確認用用のダイアログは.NET MAUI標準ライブラリ内の\verb|DisplayPrompt|を使用し、確認を表示する。
      ...
      \item \textbf{編集}：
      \begin{enumerate}
            \item 対象CTM要素の種別を判定する。
            \item 対象CTM要素の種別に応じて、編集用入力欄を提供する。
      ...
      \begin{enumerate}
            ...
            \item プロジェクト管理部から\verb|FolderItems|を受け取り、ユーザ操作監視部内で保持している\verb|FolderItems|を更新する。
            \item 描画部へフォルダ描画要求(\ref{sec:FolderItem}節を参照)を通知する。
      \end{enumerate}

      \item \textbf{新規ファイル作成}：
      \begin{enumerate}
            \item ファイル名入力用ダイアログを表示しファイル名を取得する。
            ファイル名入力用のダイアログは.NET MAUI標準ライブラリ内の\verb|DisplayPrompt|を使用し、入力欄が一つのダイアログを表示する。
...
本処理部の入力は、ユーザ操作監視部(\ref{sec:UserOperationMonitoring}節を参照)による要求や\verb|elements|、および、プロジェクトフォルダである。
出力は、解析部、および、変換部へ渡すMarkdown文字列、GUI要素生成部へ渡すGUI配置情報(JSONファイル)、\verb|elements|、プロジェクトフォルダへ出力する各ファイル(Markdownファイル、VDM++ファイル、JSONファイル)である。
...
選択フォルダの絶対パスを取得してプロぜくと管理部で保持する状態変数 \verb| SelectedFolderPath| に保持し、
...
解析部、および、変換部で処理の対象となるその文字列を保持する処理である。
...
ファイル選択処理を行う前までは、表示パターンフラグのすべての変数に\verb|false|を入力している。
...
\begin{enumerate}
  ...
  ユーザ操作監視部、および、画面切り替え処理からのファイル選択要求を受け取り、
  ...
  選択対象 Markdown ファイルと同名のCTM要素の配置情報記した、
  CTM要素配置情報ファイル
  ...
  \item GUI要素生成部の呼び出し
      ...
      CTM要素の配置情報を設定した上で、\verb|elements|をプロジェクト管理部へ返す。

      \item \verb|elementsの出力|

      GUI要素生成部から受け取った\verb|elements|をユーザ操作監視部に出力する
\end{enumerate}

\subsection{ファイル更新処理}\label{sec:FileUpdateProcess}
ファイル更新処理は、
ユファイル更新要求をユーザ操作監視部から受け取り、ユーザ操作監視部が保持する\verb|elements|を参照し、
変換部から受け取った
...
\begin{enumerate}
  ...
  ユーザ操作監視部から参照した\verb|elements| を入力として、
  変換部の
  ...
  ファイルが存在しない場合は新規作成する。
  また、\verb|VdmContent|はユーザ操作監視部が内部で保持するため、
  ユーザ操作監視部へ出力する。
...
画面切り替え処理は、ユーザ操作監視部から画面切り替え要求(\ref{sec:UOM_DoubleClick}節を参照)、
...
\begin{enumerate}
      ...
      \item 2.で作成したパスの空のMarkdownファイルを作成する。
      \item フォルダおよびファイル探索処理へ探索要求を出力し、\verb|FolderItems|を更新する。
      \item ファイル読み込み処理へファイル読込要求、および、2.で作成したパスを出力し、編集対象のMarkdownを新規作成したMarkdownにする。
\end{enumerate}
...
本処理部の入力はプロジェクト管理部から受けとるMarkdown文字列であり、出力はプロジェクト管理部に\verb|elements|を渡す。
...
本研究では、自然言語仕様記述ルール(\ref{sec:Specrule}節を参照)に基づき、Markdown仕様中で「\verb|###|」で始まる行を見出し行と呼ぶ。
...
\begin{table}[tp]
\centering
\caption{リスト3.1のMarkdown仕様記述例から生成するGUIElement一覧}
...
\begin{table}[tp]
\centering
\caption{リスト3.1のMarkdown仕様記述例から生成するEventBranch}
...
本処理は、Markdown仕様から、CTM要素である、画面要素、タイムアウト要素、ボタン要素、イベント要素、および分岐イベント要素を抽出し、
...
ただし、解析部が出力する \verb|elements|の \verb|GUIElement| は、以下のデータを保持する。
\begin{itemize}
...
\begin{enumerate}
      ...
      空の\verb|elements|を作成し、2.の先頭行判定結果より、以下の2つの場合分けを行い、GUIElementの生成を行う。
...
同時に、対象とするMarkdown仕様に対応する
GUI配置情報(JSON)をプロジェクト管理部から受け取る。
GUI要素生成部は、この\verb|elements| 、および、GUI配置情報をもとに、
画面上で操作可能な\verb|elements|を生成する処理部である。

本研究では解析部およびGUI要素生成部の双方で同一の \verb|GUIElement| クラスを用いる。
GUI要素生成部が出力する\verb|elements|の\verb|GUIElement|は、以下のデータを追加で保持する。
\begin{itemize}
...
出力は、プロジェクト管理部に返す\verb|elements|である。

GUI要素生成部で用いる通常ノードの描画仕様を表\ref{tab:gui_node_spec}に、
分岐イベントにおける可視ノードの描画仕様を表\ref{tab:gui_node_branch}にそれぞれ示す。
...
CTM配置情報が存在しない場合、または該当要素の座標が存在しない場合は、
CTM要素ごとに定義する初期配置規則に基づいて決定する。
...
\begin{enumerate}
      ...
      \begin{enumerate}[label=\roman*.]
            ...
            \item 座標を決定する。
            JSONに一致する座標が存在する場合はそれを設定し、
            存在しない場合は画面左上付近の固定位置となるよう、表\ref{tab:gui_node_spec}に示す既定オフセットを適用して設定する。
      ...
      \begin{enumerate}[label=\roman*.]
            ...
            座標は JSON を優先し、
            JSONが存在しない場合は対応するボタン要素またはタイムアウト要素の右側に来るように初期配置し、\verb|elements|へ追加する。
            \item 条件分岐を含む場合は、
            親イベント要素を表示対象外として扱い、
...
本処理は、GUI要素生成部(\ref{sec:GUIElementGenerationComponent}節を参照)で生成した\verb|elements|、お
...
から参照する\verb|elements|をもとに、操作ボタン領域に設置するボタン、フォルダツリー表示領域に表示するフォルダツリー、CTM領域にCTMを描画、および、再描画する処理部である。
...
描画部は、
各CTM要素を外接矩形として扱い、
要CTM素種別に応じた形状、寸法、および、描画スタイルを適用する。
各要素種別に応じた寸法および形状は、
...
描画部は以下に示す5つの処理から成る。
\begin{itemize}
      \item 初期描画処理(\ref{sec:first-draw}節を参照)
      \item 再描画処理(\ref{sec:Re-Draw}節を参照)
...
初期描画処理は、ユーザ操作監視部から受けとった初期描画要求、および、\verb|elements|をもとに、
初期状態のCTMを生成する処理である。

本処理では、.NET MAUI(\ref{sec:NET_MAUI}節を参照)が提供する
\verb|GraphicsView| および \verb|IDrawable| インタフェースを用いて描画を行う。

本処理の流れを、以下に示す。
\begin{enumerate}
    \item CTM領域を初期化し、背景を消去する。
    この処理は、\verb|GraphicsView|の
    \verb|IDrawable.Draw()| メソッド内で、
    \verb|Microsoft.Maui.Graphics| が提供する描画コンテキスト
    \verb|ICanvas| を用いて行う。
    \item \verb|elements|を走査し、各CTM要素の種別\verb|GUIElement.Type|を判定する。
    \item 要素種別に応じて、矩形、楕円、菱形の描画形状および描画スタイルを決定する。
    形状描画には、
    \verb|Microsoft.Maui.Graphics| が提供する
    \verb|DrawRectangle|、
    \verb|DrawEllipse|、
    \verb|DrawPath|といったAPIを用いる。
    \item 各CTM要素が保持するデータをもとに\verb|ICanvas| を用いて
    CTM領域上へCTM要素、およびラベル文字列を描画する。
    文字列描画には \verb|Microsoft.Maui.Graphics| が提供する\verb|DrawString| API を用いる。
    
    \item 遷移関係を表す要素については、解析部で設定した\verb|GUIElement.Target|を参照し、
    接続元および接続先ノードの外接矩形から矢印の開始点および終了点を算出した上で、
    遷移を示す矢印を描画する。
    \item 条件分岐を持つ要素\verb|GUIElement.IsBranch == true|については、
    親イベント本体を描画せず、
    条件分岐構造を展開して描画する。
    具体的には、以下の手順で描画を行う。
    \begin{enumerate}[label=\roman*.]
      \item 分岐条件を示す菱形を描画する。
      この描画には
      \verb|Microsoft.Maui.Graphics| のパス描画 API を用いる。

      \item \verb|Branches| の要素数 $n$ を取得し、
      縦方向に $n$ 個並べて配置する。
      各分岐証券は同一サイズの外接矩形として扱い、
      描画位置を数式\ref{eq:branch_y}に基づいて算出する。
      基準Y座標を $Y_{\mathrm{base}}$、 \verb|EventBranch|の垂直間隔を 
      $\Delta Y$(分岐先イベントの高さを含む定数)とすると、 
      ...
      \item 分岐条件から分岐先イベントへは、
      外接矩形情報をもとに
      矢印の開始点および終了点を算出し、
      線分および矢印を描画する。
      描画には \verb|DrawLine| および
      矢印描画用の補助処理を用いる。
      分岐先が画面要素として解釈できない場合は、
      描画色や線種を変更することで参照不整合を示す。

      \item 分岐条件は編集対象の単位となるため、
      描画と同時にヒットテスト用の外接矩形情報を生成し，
      \verb|GUIElement| および分岐番号 $i$ に対応付けて保持する。
    \end{enumerate}
...
矢印の開始点および終了点は、接続元CTM要素および接続先CTM要素の外接矩形情報をもとに算出する。
...
\begin{enumerate}
    ...
    \item 更新対象となる\verb|elements|をユーザ操作監視部から参照し、座標情報および選択状態などの内部状態を反映する。
    \item \verb|GraphicsView| に対して
    \verb|Invalidate()| を呼び出し、既存の描画内容を消去する。
    \item 初期描画処理と同一の手順により、全GUI要素および遷移関係を再描画する。
    \item ユーザ操作監視部により選択中と判定した要素については、
    \verb|Microsoft.Maui.Graphics| の描画属性を変更し、
    枠線の色や太さを変更することで選択状態を強調表示する。
    \item VDM\texttt{++}処理へVDM\texttt{++}描画要求を出力する。
\end{enumerate}
...
具体的には、\verb|Button| クラスおよび
それらを配置するための \verb|Layout| コンテナを使用する。
...
\begin{enumerate}
  ...
  各ボタンには、CommunityToolkit.Mvvm
  (\ref{sec:CommunityToolkit.Mvvm}節を参照)の
  \verb|RelayCommand| で定義した編集コマンド(\ref{sec:UOM_EditCommand}節を参照)を関連付ける
  \item 生成した操作ボタン群を、
  ツール上部の操作ボタン領域に配置する。
  配置には \verb|StackLayout|の
  レイアウトコンテナを用い、既存のボタンが存在する場合は
  一度クリアした上で再配置する。
  \item 選択中CTM要素の変更や表示パターンの遷移が発生した場合には、
  表示パターンフラグを更新する。
  そのたびに、操作可能なボタン集合を再取得し、
  本処理を再実行することで、
  操作ボタンの表示内容を更新する。
\end{enumerate}
...
\begin{enumerate}
  ...
  \verb|IsVisible| が \verb|false| の場合、
  対応する行は \verb|CollectionView| 上で非表示となる
  これにより、フォルダの展開、および、折りたたみ操作に伴う表示更新を\verb|CollectionView|への再描画要求のみで反映できる。

  \item 表示更新

  \verb|FolderItems| の内容や各属性値を更新した場合には、
  \verb|CollectionView|により
  フォルダツリー表示が自動的に更新する。
  これにより、フォルダ構造の変更や選択状態の更新を
  即座に画面へ反映できる。
\end{enumerate}
...
本処理では、.NET MAUI(\ref{sec:NET_MAUI}節を参照) が提供する
テキスト表示用 UI コンポーネントを用いてVDM++表示領域に描画する。
