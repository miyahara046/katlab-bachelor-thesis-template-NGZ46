...
仕様データの永続化、解析、GUI要素データの生成、および、Markdown形式からVDM++形式への変換を担う。
...
JSONファイルの出力例を、リスト\ref{lst:JSON_example}に示す。このJSONファイルには、GUI要素の名称とCTM領域での座標情報を含み、自然言語仕様記述ルール(\ref{sec:Specrule}節を参照)に含まない位置情報のみを保持する。
...
\begin{enumerate}
      ...
      \item 2.で作成したパスへ空のMarkdownファイルを作成する。
...
解析部は、記述ルールに従ったMarkdown仕様(リスト\ref{lst:markdown_example}を参照)の文字列を解析し、CTM(\ref{sec:CTM}節を参照)表示、および、操作の基礎となる構造データを生成する処理部である。
...
また、分岐イベント要素はGUIElementに加え、分岐条件とイベントを格納するリスト\verb|EventBranch|を用いてデータを保持する。\verb|EventBranch|のデータ構造を、表\ref{tb:EventBranch}に示す。
...
分岐イベントにおける可視ノードの描画仕様を表\ref{tab:gui_node_branch}に示す。
...
\begin{enumerate}
      ...
      \begin{enumerate}[label=\roman*.]
            ...
            具体的には \verb|Branches| が1つ以上存在する場合を分岐イベントとする。
            \item 条件分岐を含まない場合は、
            ...
            分岐イベントにおける Condition ノードおよび Target ノードの
            具体的な描画位置、形状の頂点位置、
            および分岐線の描画に関する幾何的計算は、
            GUI要素生成部では行わない。
...
\begin{enumerate}
  ...
  \item 分岐イベントの場合は、分岐領域(branch)部分に対する右クリックを区別し、
      ...
      \begin{enumerate}
            ...
            \begin{itemize}
                  ...
                  \begin{enumerate}
                        ...
                        分岐条件文と遷移先イベントを取得する。
                        \item 新しい分岐イベント要素(\verb|Type=Event, Branches=null|)を生成してGUI要素リスト\verb|elements|に追加する。
