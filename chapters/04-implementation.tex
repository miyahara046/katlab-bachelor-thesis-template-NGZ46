...
本研究の拡張で追加した部分も既存の\tool 同様にMVVMアーキテクチャ（\ref{sec:MVVM}節を参照）を採用している。MVVMアーキテクチャ実装のために本研究では
...
本処理部でのファイルシステム入出力には 、System.IO （\ref{sec:System.IO}節を参照）を用いる。
...
\begin{enumerate}
  ...
  \item 画面索引辞書（\verb|screenIndex|）の設定
      ...
      画面索引辞書（\verb|screenIndex|）へ対応関係を登録する。本研究で画面クラス名とは画面クラスのクラス名のことである。
      ...
      本研究では、Markdown仕様の先頭行が「\verb|# 画面一覧|」で始まる場合は画面一覧クラスと呼び、
      「\verb|##|」で始まる場合は画面クラスと呼ぶ。
...
本処理では、
System.IO（\ref{sec:System.IO}節を参照）、
および System.Text.Json（\ref{sec:JSON}）
を用いて、
各種ファイルの保存を行う。
...
\begin{enumerate}
  ...
  書き込み、保存する。
  保存する際、画面クラス名が変わっている場合は\verb|SelectedFilePath|（\ref{sec:PM_ScreenTransitionProcess}節を参照）の画面クラス名を変更する。
...
画面切り替え処理は、ユーザ操作監視部から画面切り替え要求（\ref{sec:UOM_DoubleClick}節を参照）、および、切り替え先画面クラス名を入力として受け取り、
...
ファイル読み込み処理へファイル読込要求、フォルダおよびファイル探索処理へ探索要求、および、プロジェクトフォルダへ新規Markdownファイルを出力する処理である。

本処理の流れを以下に示す。
\begin{enumerate}
      \item ユーザ操作監視部からファイル作成要求、および、ファイル名を入力として受け取る。
      \item \verb|SelectedFolderPath|をもとに新規Markdownファイルのパスを生成する。
      \item \verb|SelectedFilePath|へ新規Markdownファイルのパスを格納する。
      \item 指定したパスへ空のマークダウンファイルを作成する。
...
本処理部では、System.Text.RegularExpressions（\ref{sec:System.Text.RegularExpressions}節を参照）を用いて Markdown文字列の各行を解析する。
      ...
      \begin{itemize}
            ...
            \begin{enumerate}[label=\roman*.]
                  ...
                  \begin{enumerate}[label=\alph*.]
                        ...
                        抽出したボタン名について、\verb|HashSet<string>|（\ref{sec:System.Collections.Generic}節を参照）を用いて重複判定を行う。
...
必要な入力値（名称、遷移先、条件文）をダイアログで取得した上で、
GUI要素リストを更新する処理である。
