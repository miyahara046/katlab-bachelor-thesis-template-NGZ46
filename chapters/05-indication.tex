...
リスト\ref{lst:T-Screen}に示した画面仕様のCTMを表示したCTM領域を図\ref{fig:T-CTM-Screen}に示す。
...
この故意に誤りを加えた画面仕様のCTMを表示したCTM領域を、図\ref{fig:T-CTM-Screeneror2}に示す。
...
次に、図\ref{fig:T-Tree-2}に示したフォルダツリー表示領域内の「BOX数入力画面.md」を左クリックする。「BOX数入力画面.md」を左クリックした直後の\VDM 仕様表示領域の一部を、図\ref{fig:T-VDM-2}に示す。
\begin{figure}[tp]
  \centering
...
\begin{figure}[tp]
  ...
  \caption{「BOX数入力画面.md」を左クリックした直後の\VDM 仕様表示領域の一部}
  \label{fig:T-VDM-2}
\end{figure}
...
\begin{figure}[tp]
  \centering
  \includegraphics[width=0.7\linewidth]{./images/T_11.png}
